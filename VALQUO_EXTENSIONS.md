...
| V6 | Dip Detector - the testable claim behind the tab | edge lane | **DONE 2026-08-13 - NULL on all four arms.** `PREREG_v6_dip_detector.md` committed ALONE at `93e3e60`, **before the tab existed and before any of its copy was written**; `HANDOFF_edge_audit.md` V6. A 20% and a 30% drawdown from a 252-day trailing high, crossed with 63d and 126d, conditioned on the panel `quality` theme above its midpoint AND a point-in-time health score at or above the shipped scale's midpoint - each arm requiring a vs-universe leg AND a vs-UNCONDITIONED-dips leg to clear in BOTH halves against that leg's own permutation p95. **Not one arm clears either leg in both halves.** **The number the product needs is not the verdict: Spearman(drawdown, `momentum`) = +0.6642**, so a dip tab would systematically surface names the live composite is marking down. **Seven of eight leg-series flip sign between halves, all the same way** - negative 2009-2017, positive 2017-2026 - so **a Dip Detector validated on the last eight years alone would have looked like it worked**. **No cell's effect reaches its own MDE**, so NULL means could-not-be-separated, never absent. The conditioning keeps 26.8% of dipped names and they are 1.73x larger, making that leg partly a size screen. **CLOSE-OUT AMENDMENT: the app lane shipped the tab at `6b7c358` twelve minutes before this landed, carrying `valuation/web/dip_posture.py` - the same mechanism, built independently, gated on this register with a designed close-out.** So the close-out was performed on **their** module (`STATUS = NULL`, verdict fields filled, 39/39 `test_dip.py`) rather than a second one being created, and V6 touches `valuation/web/` in exactly one file - a scope change against the register's own section 8, **declared not absorbed**, because the alternative was a live surface still saying *"we are testing it"* after the register closed. The tab now calls the screen **a filter, not a forecast**. **Two defects found doing it:** their `REGISTER` cited a file that does not exist, and **`digest_eligible` was `STATUS != OPEN`, so a NULL - evidence the claim is NOT supported - would have silently unblocked an outbound push of a dip list.** Now `STATUS == POSITIVE`; **whether that list ever goes out is routed to Don.** Equity `N` 202 -> 206. |
| V6-B | Dip branch reframed as a RISK question | edge lane | **DONE 2026-08-13 - M1 REAL, the branch LIVES.** `PREREG_v6b_dip_survival.md` committed ALONE at `dc5ae98`; `HANDOFF_edge_audit.md` V6-B. After V6's four return nulls, the same population asked a RISK question instead. **Among 20%+ dips the HEALTHY set - V6's floors UNCHANGED and imported - falls a further 20% within 126d on 32.51% of occasions against 43.35%, a 10.8pp absolute and 25.0% relative reduction at HAC *t* -10.58, replicated in both halves and clearing the pre-committed 3.0pp economic floor threefold.** The effect runs 4-5x its own MDE, the opposite of V6 and S19. **It is NOT a size sort**: healthy dips are 2.06x larger, so C8 was deepened into a within-size stratification (post-hoc, no verdict) and 5 of 5 quintiles are negative with 4 of 5 clearing both halves - **but the gradient runs -14.3pp in the smallest quintile to -3.8pp in megacaps, so the claim is strongest exactly where the shipped book is not.** **THE WORD "DIED" IS NOT EARNED**: the metric that separated is a further -20% (deepening, not dying), while M2 - the actual bankruptcy metric - is **VOID on power** at 42 events against a pre-committed floor of 60. **Arm 2 (top-decile overlay) NULL on both horizons; Arm 3 (insider open-market buying) NULL with abundant coverage.** Premise that rewrote the task's own metric: **82.63% of delistings here are ACQUISITIONS** - and a correction against my own register, the UNHEALTHY set is acquired MORE (0.604x), so a naive P(delisted) would have exaggerated the healthy set's advantage, not reversed it. **`V6-OPT` unlocks**, inheriting three caveats. ADOPTS NOTHING. Equity `N` 206 -> 212. |
| V6-OPT | Cash-secured puts on healthy dips | options bot | **DONE 2026-08-13 - stage 1 GATE OPEN, stage 2 REJECTED.** `PREREG_v6opt_csp.md` committed ALONE at `88685c9`, fixing BOTH stages before either ran; `HANDOFF_optionsbot.md` section 57. **Stage 1 (descriptive, no arms) found real richness**: post-dip implied vol sits **+17.71%** above the name's own trailing 252-day median against **+13.29%** for unhealthy dips - so the market does **not** price the M1 distinction - a 25-delta 30-45 DTE put pays a median **2.550% of strike** over 32 days (**27.73% annualised**) net of the shipped fill engine at the touch, and the variance risk premium is **+0.0391** vol points. The elevation decays away almost entirely within 30 trading days (**-16.56%** by t+30). **Stage 2 REJECTED on the discriminator.** The healthy arm is positive in both halves (**+1.1342%/trade**, 84.4% win rate) and beats random entry decisively (**z +7.25, p 4.2e-13** on 457 paired name-year cells) - but the **identical trade on UNHEALTHY dips earns MORE (+1.2651%) and beats it in both halves**, so the health floors do no work and the trade is just short vol, which `A3` already rejected at -7.99%/trade. **THE MECHANISM IS THE FINDING: a 25-delta put IS a 25% assignment probability BY CONSTRUCTION**, so assignment comes back 25.30% healthy vs 25.73% unhealthy while the unhealthy name pays more premium because its vol is higher - **the strike selection has already spent the risk difference**, and a delta-targeted strategy is blind to a risk edge the option market has priced into the strike. **And the edge is a quarter of its headline where it can be traded**: M1 re-measured on the covered rows is **-2.797pp** against V6-B's -10.84pp, because options exist on the large end (8.26x market-cap tilt) and V6-B measured the separation weakest exactly there. Coverage forbids a full-panel gate for the fourth time (12.78%, 40 of 69 dates, 28 of 29 gaps EARLY). **A PREMISE FINDING CORRECTS `U6`**: the chain cache is **not** 100% calls - it holds 1,288,750 puts against 1,288,751 calls - so half of U6's stated blocker was false; its coverage blocker stands. ADOPTS NOTHING. Options `N` 287 -> 292; equity untouched by this item (218 after the merge). |
