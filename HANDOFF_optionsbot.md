...
## 68. MB16 — quote-classified VPIN: the kill does not fire, the arm is NULL, and the register's own comparison is blind (2026-08-19)

`PREREG_mb16_vpin.md` committed **ALONE at `7fa88e5`**, markdown only, zero `.py`, a strict
ancestor of every commit that scores an outcome; **1 options trial booked at `2e7bf92` BEFORE the
arm ran, `N` 304 -> 305**. **ADOPTS NOTHING. `R2` stands and `O11` binds.**

### 68.1 Only the quote-classified version is built, and that is the point of the item

Andersen & Bondarenko's critique of Easley-López de Prado-O'Hara's VPIN names its target
precisely: the **Bulk Volume Classification** scheme, which they find inferior to a standard tick
rule, with VPIN predicting volatility largely because rising volatility induces systematic BVC
classification errors. **The contested component is the classifier** — and that is the component
this project already does properly. `O14` built a quote-based Lee-Ready classifier and measured it
classifying a median **98.54%** of eligible prints. So the BVC classifier is **built nowhere**,
pinned by an AST test.

### 68.2 THE INSTRUMENT WAS VALIDATED BEFORE THE HYPOTHESIS, AND IT IS EXACT

`MB15`'s lesson applied. The gating control ran and was **read before the kill**, not after:

* the classifier reproduces `O14`'s banked `classified_rate` median **0.9854035696220825 to all
  sixteen digits**;
* per-unit `signed_volume` reproduces `O14`'s banked values at **max absolute deviation 0.0 across
  all 3,863 units**.

So VPIN is built on `O14`'s instrument rather than on a lookalike, and every comparison against
`O14`'s features means what it says. The arm **refuses to run** without a passing kill artifact —
mutation-tested 4 of 4 (kill reported as fired, either control leg flipped, artifact deleted),
with the artifact restored byte-for-byte.

### 68.3 THE KILL DOES NOT FIRE

| comparison | pooled | within-month |
|---|---|---|
| `signed_volume` | **−0.0683** | −0.0429 |
| `unusual_volume` | −0.0348 | −0.0372 |
| **registered kill statistic** | **0.0683 against a bar of 0.90 → DOES NOT FIRE** | |
| **\|signed_volume\|** — *not the registered comparison* | **+0.5661** | +0.5357 |

### 68.4 TWO DEPARTURES FROM THE ITEM'S WORDING, DECLARED IN THE REGISTER RATHER THAN MADE SILENTLY

**(1) The kill's "within date" cross-section does not exist on this book.** Measured: **1,570
dates, median 2 names per date, max 17, ZERO dates reaching 20, and 39.7% carrying exactly one
name.** A within-date Spearman is undefined at n = 1 and identically ±1 at n = 2 — it would be a
constant or noise, never a measurement. **`O14` hit the identical wall and sorted MONTHLY**, so the
kill is taken within month with the pooled figure beside it and **fires if EITHER exceeds the
bar**, which is stricter than either alone.

**(2) THE REGISTERED COMPARISON CANNOT DETECT THE RENAMING IT EXISTS TO CATCH.** VPIN is
**unsigned** — mean `|buy − sell|` per equal-volume bucket — and `signed_volume` is **signed**.
Flipping every aggressor side leaves VPIN bit-identical and negates `signed_volume`, both pinned
by test. So correlating the two cannot see a VPIN that is a pure function of that feature's
**magnitude**, which is exactly the renaming the kill is for.

**The rule was run AS WRITTEN and the defect is reported rather than repaired** — `MB1`'s
discipline. **And here the blind spot concealed nothing**: the magnitude comparison reads
**+0.5661**, eight times the registered statistic and still comfortably inside the bar, so roughly
two-thirds of VPIN's rank variance is information neither `O14` feature carries. **A register can
be wrong about what its own statistic measures and right about the answer, and both halves have to
be said.**

### 68.5 THE ARM IS NULL, ON ALL THREE LEGS

`O14`'s `score_arm` imported verbatim — the identical monthly-quintile long-short, month-block *t*
and within-month permutation null that `O3`/`O4`/`O5`/`O14` were judged by. **n = 3,863 over 115
months**, split 2020-12, two-sided.

| | Q1 | Q2 | Q3 | Q4 | Q5 |
|---|---|---|---|---|---|
| mean return | +0.0205 | +0.0504 | +0.0147 | −0.0625 | −0.0631 |

* **full sample:** long-short **+0.0835**, *t* **+1.7345** against its own permutation p95 of
  **2.0005** — **fails**; CI95 **[−0.0088, +0.1784]**, straddling zero.
* **early:** ls +0.0682, *t* +0.9262 vs p95 2.0180 — fails.
* **late:** ls +0.0992, *t* +1.5943 vs p95 2.0498 — fails.
* **the halves DO agree in sign** — the one leg it passes.

**Direction, stated with its verdict and carrying no claim:** low-VPIN alert-days earn **8.35pp
per trade more** than high-VPIN ones, and the two bottom quintiles are the only negative cells.
That is the *opposite* of a naive "informed flow is on the alert's side" story — **and it is NULL,
so no directional claim is made.**

### 68.6 THE NULL CARRIES ITS OWN RESOLUTION, AND THE EFFECT IS CLOSE RATHER THAN ABSENT

`V6` and `S19`'s rule: a NULL means *"could not be separated from zero at this resolution"*, never
*"absent"*. Derived from the arm's **own** standard error (0.04817) and its **own** calibrated bar:

* **MDE at the arm's own p95 bar: +9.64pp.** Observed: **+8.35pp.**
* **So the observed effect sits BELOW its own detection threshold — by 1.3pp.**

**Quote the MDE with the verdict or do not quote the verdict.** This is a design that could not
have returned a positive verdict for an effect of the size it actually saw.

### 68.7 Diagnostics, no verdict

* **The bucket count is not load-bearing**, which is the best evidence the construction is not an
  artefact of n = 50: rank correlation against the primary is **0.9493 (n=10), 0.9800 (n=20),
  0.9929 (n=100)**.
* VPIN's own distribution: p05 **0.1438**, median **0.4160**, p95 **0.8076**.
* **VPIN is the SECOND-STRONGEST of the six flow features ever measured on this book, and all six
  are NULL:** `sweep_share` 3.0607, **VPIN 1.7345**, `unusual_volume` 1.5107, `signed_volume`
  0.6577, `block_share` 0.4366, `pc_flow_imbalance` 0.2556.
* **ρ against `sweep_share` is −0.5180** — substantial and **NEGATIVE**, i.e. toxic flow carries
  *fewer* sweeps. Reported because it refutes half my own registered expectation (§68.9).

### 68.8 REPORTED OUTSIDE THIS ITEM (`RUN_RULES` rule 3), AND HALF OF IT IS THIS SESSION'S OWN

**`research_log._parse` has NO DEDUP BY ID, so an item that books a `PRE-REGISTERED` row and later
appends a separate verdict row is counted TWICE.** Measured:

* **`MB1` carries 2 counted rows at `n=2` and charged 4 options trials against a record that says
  2** — that is this session's own work, from earlier today;
* **`O21-D2` carries 3 counted rows at `n=1` and charged 3 against a record that says 1.**

**Options `N` is therefore OVERSTATED BY 4 — 305 as reported against 301 distinct searches.**
The direction is the **safe** one, because a larger `N` raises every bar, which is why nothing
broke and no published claim moves. **It is deliberately NOT corrected here:** lowering `N` moves a
denominator every published options claim is gated on in the **permissive** direction, and it
touches another lane's rows, so it needs its own decision. **MB16 books ONE row and its verdict
cell is EDITED IN PLACE rather than duplicated**, which is the fix for the behaviour rather than
for the count.

### 68.9 Expectations: 3 right, 1 wrong, 1 split

| # | prediction | outcome |
|---|---|---|
| E1 | A1 is NULL (85%) | **RIGHT** |
| E2 | it fails the both-halves leg rather than the full-sample leg (60%) | **WRONG** — it fails all three |
| E3 | VPIN's \|t\| exceeds `signed_volume`'s +0.6577 (60%) | **RIGHT** (1.7345) |
| E4 | the quintile means are non-monotone (70%) | **RIGHT** (Q2 > Q1) |
| E5 | ρ against `sweep_share` above 0.30 (55%) | **SPLIT** — 0.518 in magnitude, but NEGATIVE |

### 68.10 Scope, and what is not done

**ALERT DAYS ONLY.** The tick cache is exactly the alert days, so every figure is conditioned on
them and **none generalises to the tape**. There is **no pinned freeze for the tick cache** —
`D:` holds only the two CHAIN freezes — so the units are pinned by a recorded fingerprint
(`MB15`'s finding, inherited).

**NOT DONE, named so it is not mistaken for done:** the **BVC** version is not built and MB16's
verdict says nothing about it; `O14`'s five arms are **not reopened** and stay NULL; no incremental
test of VPIN against `|signed_volume|` was run (that is a second arm and would be a second trial);
...
## 70. S3-I3 — the assignment and margin model for short-side forward books (2026-08-23)

**ZERO TRIALS, `FIXED`-class.** No hypothesis, no bar, no verdict against a threshold — an
instrument, the `S25`/`MB15`/`I-2`/`I-3` class. `by_domain` is bit-identical across the log
append (equity 242, options 305, infra 20) and `rows_fixed_not_counted` rises, which is the
proof the row was seen and correctly excluded. **ADOPTS NOTHING, LICENSES NO TRADE, AND NO
BOOK DECLARES ON IT YET** — `O11` binds, sandbox only.

**ROUTED HERE RATHER THAN OWNED HERE.** `SEASON3_MAP.md` assigns S3-I3 to options-bot; that
lane is mid-queue, so this lane built it. The recorder, the F-books and S3-I1 itself remain
options-bot's, and this section exists so the module is found rather than rebuilt.
...
* `assignment_at_expiry` — assigned per MONEYNESS, strict inequality, matching
  `csp_surface.settle_put`'s `s < k` and `paper_track`'s MA36 guard. Returns the per-share
  obligation, the share delta and the cash delta.
* `secured_cash` — Reg-T CASH-SECURED and COVERED-CALL conventions. This is S3-I1's *"the
  secured cash is the denominator of every return quoted"*, and it is the whole reason a short
  book cannot reuse `options_sizing`, which makes the PREMIUM the capital at risk.
* `early_assignment_flag` — `O21`'s machinery IMPORTED, never re-derived.
* `validate_declaration` — S3-I1 §4's *"a short book without the assignment module is
  REFUSED"*, in code.

**IT LIVES IN `valuation/edge/` AND THAT WAS FORCED, NOT PREFERRED.** MA23's boundary test
forbids any non-study module importing `valuation.studies`, and the S3-I1 recorder is
`paper_track` lineage, i.e. engine. A `studies/` placement would have been unimportable by the
one caller that must call it. Pinned by test, with the reason in the test's own name.

### The premise correction, and it is checkable rather than a matter of taste

**`SEASON3_MAP.md` gives THREE mutually inconsistent lists of which books consume S3-I3, and
the S3-I3 row's own list is the wrong one.**

* Its per-book *"waits on"* column names **eight**: F-4, F-6, F-7, F-8, F-10, F-17, F-18, F-20.
* Its §5 dependency graph names **seven** — the same set minus **F-6**.
* **S3-I3's own row names five: F-4, F-10, F-11, F-12, F-19** — and the intersection with the
  per-book column is only F-4 and F-10.

The documentary fact settles it without needing my reading: **F-11, F-12 and F-19 do not claim
I3 in their own rows.** As corroboration, and labelled as my reading rather than the map's:
F-11 and F-12 are long-option structures with no short leg to assign, and F-19 is *"a gate, not
a book"*. F-6 is a zero-cost COLLAR, whose short call leg is exactly what this module exists
...
Five controls on V6-OPT's own 660 settled cash-secured puts, joined to its stage-1 events for
strike and credit.

* **C1 — B7 FIDELITY, EXACT.** `settle_short` reproduces `csp_surface.settle_put` AND V6-OPT's
  published per-trade `ret_on_strike` on **660 of 660 rows at max |delta| 0.000e+00**, zero
  assignment mismatches. The COUNT is gated, because `MB21`'s C1 scored a perfect 0.000e+00 on
  an empty frame by comparing nothing.
* **C2** — 660 reproduces the artifact's own published `n`, so this is V6-OPT's book.
* **C3 — THE SPLIT TRAP IS LIVE ON THIS BOOK AND IT IS NOT MARGINAL: re-settling every trade
  against the ADJUSTED close instead of `raw_close` FLIPS 192 of 660 assignment verdicts,
  29.1%.** Median absolute basis gap **5.03%**, and **half the rows differ by more than 5%**.
  The worst is **CMG at expiry 2016-11-18: `raw_close` 411.80 against an adjusted 8.236**, a
  50x gap on a 365 strike — the adjusted basis books an obligation of **97.7% of the strike on
  a trade that actually expired worthless.** This is the measurement that turns *"remember to
  use `raw_close`"* from folklore into a number.
* **C4 — the MA36 mirror, on real rows.** **493 of the 660 expired worthless and every one owes
  EXACTLY zero.** Inheriting MA36's long-side rule unchanged would have booked all 493 at
  **-100%** — a short book's best outcome recorded as a total loss, on three quarters of its
  trades.
* **C5 — early-assignment census, DIAGNOSTIC, NO VERDICT.** 113 of 454 trades with a dividend
  history span an ex-date. These are PUTS, where a dividend DISCOURAGES early exercise, so the
  census sizes the exposure for the CALL books (F-6, F-7, F-18) and says nothing about these
  rows. The flag reports RATIONALITY, never a probability; no assignment rate may be read off
  it.

### A defect in my own instrument, caught by my own gate on the first real row

`settle_short` first computed the return as `pnl_total / secured_cash`, i.e.
`(pnl x 100) / (k x 100)`. **That is not the same float as `pnl / k`** — the multiplier
round-trip loses a bit — and `settle_put` divides by the bare strike. The B7 assertion fired
immediately on V6-OPT's first row, at the last digit (…246308 against …246304).

**The fix was not a tolerance.** A tolerance would have hidden the fact that the two were doing
different arithmetic. `secured_cash` now also returns `secured_per_share` and the ratio is taken
per share, so the identity is exact BY CONSTRUCTION rather than within an epsilon.

**And the reason the tests missed it is the portable part: every one of my six original fidelity
cases used round numbers**, on which the scaled and unscaled forms happen to agree bit-for-bit.
Real strikes and fifteen-significant-figure credits broke it on contact. **A float identity
tested only on round numbers is untested**; four messy cases from the real book are now in the
suite, including the row that caught it.

### Two defects in my own TESTS, both found by mutation rather than by reading

1. **The required-field test read the very constant it was supposed to pin.** It iterated
   `SB.REQUIRED_SHORT_FIELDS`, so deleting a field from the tuple silently deleted its test too.
   Repaired with `MA13`'s committed-literal idiom. *(The mutation that exposed it dropped
   `spot_basis`, which turns out to be behaviour-PRESERVING because a second check catches it
   downstream — so the honest statement is that the pin, not the refusal, is what now bites
   there. Two further mutations dropping `assignment_model` and `early_assignment_flag`, which
   have no downstream check, are caught by the refusal itself.)*
2. **The O21-imported test banned a NAME while the defect that happens is an INLINE
   re-derivation.** `max(0.0, k - s)` written in place defines no function and slipped straight
   through. This is the substring-ban family in a new costume — `never-ban-a-substring` for the
   Nth time — and the repair asserts the POSITIVE property instead: `DIV.intrinsic` and its four
   siblings must be CALLED, and `assignment_at_expiry` must obtain the obligation from
   `DIV.intrinsic` exactly once.

### A third defect, in the validator's own path resolution

`_data_root` first probed `os.path.isdir(data/free_analysis)` and the WORKTREE carries that
directory EMPTY, so it resolved to a root holding nothing and refused. **EXISTENCE IS NOT
POPULATION** — `DEEPITM-FIN`'s defect exactly, where an empty `data/bulk/prepared/bars`
shadowed the primary's 502 files and the run reported `spot series: 0`. It now probes for the
FILE it cannot run without, and the artifact is written to the PRIMARY root, because two items
in two sessions found an artifact stranded in a worktree that later disappeared.

### Reported outside this lane (`RUN_RULES` rule 3)

**A THIRD STRANDED ARTIFACT, AND IT IS THIS LANE'S OWN.**
`I3_CRASH_GATE_VALIDATION.json` was sitting in `.claude/worktrees/r1/data/free_analysis/` and
was **ABSENT from the primary root** — so `I-3`'s own validation output would have died with
this worktree. That is the THIRD instance in three sessions after `MA28_CARD.json` (found by
`E-5`) and `I2_BURN_IN_CENSUS.json` (found by `E-6`), and unlike those two it was written by
the same lane that found it. Copied to the primary root; `tests/test_i3_crash_gate.py` runs
**36 tests with zero skips** against it. **The pattern is now regular enough to be a rule
rather than an anecdote: an artifact written by a worktree-run script does not survive the
worktree, so every script must resolve the PRIMARY data root before writing** — which is what
`s3i3_short_book_validate.py`'s `_data_root` does, and why its own probe had to be repaired.

**`CONTRACT_MULTIPLIER = 100` is defined TWICE** — `options_fill.py:70` and
`options_sizing.py:108`. `MA5`'s four-copies-of-one-idea shape, harmless today because the two
...
* **Naked shorts are REFUSED BY NAME, not approximated.** FINRA 4210's maintenance formula is a
  different model with its own floor; a cash-secured stand-in would UNDERSTATE the requirement,
  which is the unsafe direction.
* **The stock leg of a covered call is not marked here** — the host book owns it and already
  marks it, and modelling it here would double-count.
* **No assignment PROBABILITY is estimated.** The flag says whether assignment is rational for
  the holder now. Whether a given holder acts is unobservable.
* **The OCC's real $0.01 auto-exercise threshold is NOT implemented.** This module uses strict
...
only, zero `.py`, 249 lines, a strict git ancestor of every measurement commit; **1 options trial
booked at `5423515` BEFORE any runner existed, `N` 307 -> 308**, re-read post-merge as equity 242,
options 308, infra 20. **ADOPTS NOTHING. `O11` GOVERNS and nothing here licenses a trade. `R2`
stands.**

### Why the item exists

**THE ENTIRE O-SERIES BOOK IS 100% CALLS.** `O13` measured `opt_right` CONSTANT across all 3,870
banked trades, so every options claim this project has ever made is about long calls at swing
horizon. **Long puts had never been tested.** The mechanism is `MA28-CARD`, the strongest
replicated risk result the project owns — verified here against its own banked card rather than
quoted: `rate_flagged` **0.026597**, `rate_kept` **0.008743**, `ratio` **3.0422123745999063**,
`n_flagged` **6542**. A put is the instrument that pays on exactly that event.

### 1. A PREMISE CORRECTION TO MY OWN REGISTER, forced by the data and declared before any outcome

**THE DECLARED TENOR DOES NOT EXIST IN THE FREEZE THE REGISTER NAMED.** `resolve_chains` returns
the EOD chain freeze, which carries **8 expiries per date, DTE 3–59** — nothing at 150–210 at all.
The tenor lives only in the **harvest** freeze (`resolve_harvest`, `O21-D2`'s and `DEEPITM-FIN`'s),
which carries 157/185 DTE on AAPL-2018 at `max_dte` 1200. Repointed to it rather than moving the
tenor, **because changing the tenor to fit the data available is choosing the design on the data.**
Both freezes are pinned and the shared resolver RAISES rather than falling back.

### 2. A DEFECT IN MY OWN REGISTER, AND IT RUNS ~17x THE WRONG WAY

The register says *"at the ~75% chain coverage this project measures elsewhere the design sits
close to its own floor"*. **Measured: the harvest freeze reaches 6711 of 113,945 panel cells —
5.89%.** It is a TARGETED pull (alert symbol-years plus Tier C) and the constraint stacks: 775 of
2,531 panel names present, but only **421** also carry a populated bars file. **The 75% was a
coverage figure measured on the ALERT BOOK and applied to the PANEL.** `V6-OPT`'s own lesson — a
composition fact about a BOOK is not a coverage fact about a CACHE — committed with the warning in
view.

### 3. THE KILL DOES NOT FIRE, AND IT DOES NOT FIRE DECISIVELY

6711 slices, **2844 usable (42.4%)**, 1265 name-year cells. Paired cluster
bootstrap, 2000 draws.

| threshold | flagged mean | unflagged mean | ratio | CI95 | excludes 1.0 | excludes the 2.0 bar |
|---|---|---|---|---|---|---|
| **x = 0.50** (MA28's own crash definition, PRIMARY) | 0.030643 | 0.020029 | **1.5299** | [1.1902, 1.9201] | **yes** | **yes** |
| x = 0.65 (secondary, no kill power) | 0.073555 | 0.053468 | 1.3757 | [1.1333, 1.6299] | yes | yes |
| x = 0.80 (secondary, no kill power) | 0.176759 | 0.147341 | 1.1997 | [1.0833, 1.3330] | yes | yes |

**BOTH HALVES MATTER AND NEITHER MAY BE QUOTED ALONE. The market GENUINELY DOES price the flag** —
the ratio excludes 1.0 at all three thresholds, so flagged names' options carry a measurably fatter
risk-neutral left tail. **And it prices only PART of it** — the interval sits entirely below the
kill bar at all three. The differential is **largest in the deepest tail** (1.53 -> 1.38 -> 1.20 as
the threshold shallows), which is where MA28's crash actually lives.

**THE COMPARISON THE REGISTER WAS BUILT TO MAKE: MA28's PHYSICAL crash-rate ratio is 3.0422 and
the RISK-NEUTRAL tail ratio at that same threshold is 1.5299 [1.1902, 1.9201] — the
physical figure sits ABOVE THE ENTIRE 95% INTERVAL.** That gap is the residual a put would be
buying, and it is the first time this project has priced one of its own risk findings against the
market's own quote.

**THE LIMITATION TRAVELS WITH IT AND THE COMPARISON MAY NOT STAND WITHOUT IT.** MA28's 3.0422 is a
**63-trading-day PHYSICAL** rate; this tail is a **~185-day RISK-NEUTRAL** one. Different horizons,
so the LEVELS are not comparable — only the RATIO comparison is, and **how the ratio itself scales
with horizon is UNMEASURED.** The risk-neutral measure also carries a variance risk premium that
inflates tail mass for EVERY name; the ratio is what cancels a premium common to both groups, and
whether it is common is not established here.

**`E-4`'s PRIOR WAS DECLARED IN ADVANCE AND HELD.** It measured the market tail flag and the
accounting flag NEARLY INDEPENDENT at Cohen's kappa **0.0624** and odds ratio **2.4257** (both
verified in E-4's own artifacts), which the register named IN ADVANCE as strong prior evidence the
kill would not fire. It did not. **`E-4`'s UNDERPOWERED verdict is NOT reopened.**

**The kill bar is `MA28`'s own, verified rather than asserted:** its card carries
`B2_ratio_floor: 2.0`. It was reused verbatim, not chosen here.

### 4. THE ARM IS UNDERPOWERED BY CONSTRUCTION, and the register pre-committed that branch

Sec 6 says: *"If it lands below, the honest outcome is UNDERPOWERED and the trial still charges."*

| arm | flagged | unflagged | cells | gap | CI95 | early / late | MDE80 | obs/MDE80 | power vs prior |
|---|---|---|---|---|---|---|---|---|---|
| **primary** (name-year) | 74 | 121 | 62 | -0.0675 | [-0.2301, +0.1068] | -0.1348 / **+0.0777** | 0.3020 | 0.224x | **0.0019** |
| secondary (year x cap-q) | 96 | 2490 | 20 | -0.1111 | [-0.1924, +0.0104] | -0.1371 / -0.0682 | 0.2913 | 0.381x | 0.0066 |

**THREE THINGS MUST TRAVEL WITH THOSE NUMBERS.** Both gaps are **NEGATIVE** — flagged names' puts
earned LESS, the OPPOSITE of the hypothesised direction — but each sits at **0.22x to 0.38x of its
own detection threshold** and both intervals straddle zero, so **neither carries a claim in either
direction.** The primary's **halves DISAGREE IN SIGN**, this record's most repeated pattern.
And **power against the register's own 0.07 SD prior is 0.19%** — *this design could not
have returned a positive verdict for the effect it was built to look for.*
...
**3600**. The binding constraint is the FLAGGED leg at 74 against a floor of 3600 — short
by **48.6x**.

**THE MATCHING KEY WAS AMBIGUOUS IN MY OWN REGISTER AND BOTH READINGS WERE FEASIBLE.** *"Matched on
name-year and market-cap quintile"* admits a strict reading (the same name in the same year,
flagged in one quarter and not another — 65 cells, 74 flagged retained) and a looser one (year x
cap quintile — 96 flagged retained). **The STRICT one is primary**, because it is the more literal
reading of the register's own words and the more conservative design; **taking the looser one
because it retains more n would be choosing the design on the data.** Both are reported.

### 5. FOUR DEFECTS OF MY OWN, every one caught by disbelieving a number rather than by anything
raising, and the third is the portable one

* **(a) THE x100 MULTIPLIER.** `simulate_trade` returns `net_pnl` in DOLLARS while `entry_fill` is
  the PER-SHARE premium, so the naive ratio read **+102.46 instead of +1.0246**. `EVOWN`'s pass-2
  defect exactly. **The engine's own constants proved the correction rather than my arithmetic
  asserting it**: that trade exits at `"target"` and `TARGET_PCT` is 1.00, and the stop cases land
  at -0.53 against a `STOP_PCT` of -0.50. `CONTRACT_MULTIPLIER` is IMPORTED, not typed — `MA5`
  already found it defined twice and a third would be the same defect. The identity
  `net_pnl = ret x entry_premium x 100` reproduces on the shipped book at **1.8e-12**.
* **(b) A NON-RANDOM DROPOUT FROM A DATE TYPE.** 26 of 2,688 trades were lost to `TypeError` at
  `options_backtest.py:512` because I hand-built a `bars` dict of `datetime.date` where the
  canonical cache stores **ISO STRINGS**. Not the engine's bug — its settle branch and
  `window_ending` both assume strings and the shipped cache supplies them. **Those 26 are NOT a
  random 1%: they are exactly the trades reaching the SETTLE branch**, which for a long put is
  where a deep-ITM expiry pays. Fixed by deleting my stand-in and calling the shipped
  `OB.load_bars` (`B7`). **AND A CORRECTION AGAINST MY OWN FIRST READING OF IT, made by measuring
  rather than reasoning: I said the dropout ran AGAINST the hypothesis and that fixing it would
  move the gap toward the arm. IT MOVED AWAY.** All 26 recovered trades are **UNFLAGGED**, mean
  return **+0.7799** against the book's -0.2001 — the mechanism was right and the tail belonged to
  the **CONTROL**, so the gap went -0.0528 -> -0.0675.
* **(c) THE SPLIT GUARD, AND IT IS THE ONE TO REMEMBER.** `MNST` entered 2016-10-18 on a 135-strike
  put and split **3-for-1 on 2016-11-10**; strikes are AS-TRADED and never adjusted while
  `raw_close` crosses the split, so the settle branch valued a pre-split strike against a
  post-split underlying and booked a fake **+1453%**. `U1-SPLIT`'s defect on GE's 1-for-8 reverse
  split, in a new costume. **The engine HAS the guard and I did not pass it:** `simulate_trade`'s
  `splits=` defaults to `None`, documented as *"the historical behaviour exactly"*, **so a caller
  that forgets it silently gets no guard at all.** It was not one trade — the new test fails
  against the pre-fix book at **16 traded rows spanning a split**, and in a 195-trade matched
  book one +1453% row moves the mean by roughly 7 percentage points on its own. **THE PORTABLE
  PART: a guard whose default is OFF for backward compatibility is a guard a new caller silently
  loses.**
* **(d) A REFUSED TRADE IS TRUTHY.** `simulate_trade` signals refusal as `{"ok": False,
  "reason": ...}` rather than by returning None, so `if not tr` let it through and the row landed
  marked `traded: True` with its REASON DISCARDED. **The scoring was never wrong** — those rows
  carry a null return and are filtered — **but a book that says it traded what it refused is wrong
  about its own history**, and the reason is exactly the diagnosis rule 9 exists to keep.

**FOUR BUILD PASSES FOR WHAT SHOULD HAVE BEEN ONE.** `EVOWN` paid three; this paid four, and three
of the four are my own omissions.

### 6. REPORTED OUTSIDE THIS LANE (`RUN_RULES` rule 3)

* **`valuation/studies/market_tail.tail_mass_row` takes `thresholds` as a parameter and then
  indexes a HARD-CODED `PRIMARY_THRESHOLD = 0.70` into the result** — so it RAISES `KeyError` for
  any caller whose thresholds omit 0.70, while the sensitivity loop two lines below guards
  correctly with `if frac in s.tail_mass`. **The parameter is honoured everywhere except the one
  column named for it.** Its emitted columns also come from `SENSITIVITY_THRESHOLDS =
  (0.50, 0.60, 0.80, 0.90)`, which does not contain this register's declared **0.65**, so even
  passing a superset could not emit it. **NOT EDITED** — it is `E-4`'s instrument and editing it
  would move a landed figure. This register dropped to the shipped primitives underneath it
  (`pick_expiry` + `rnd.build_slice`), both of which take thresholds as real parameters. **E-4's
  lane's to fix.**
* **`HANDOFF_optionsbot.md` carries TWO `## 73.` sections** — the fleet lane's and `EVOWN`'s —
  merged cleanly because neither side touched the other's file region. Harmless for content,
  ambiguous for citation; the same shape as the `EXPECTED_BY_DOMAIN` double-assign, where
  keep-both is right for ROWS and wrong for a SINGLE-VALUED identifier. **Not renumbered here** —
  it is another lane's section and renumbering would break anyone citing it.

### 7. NOT DONE, named so it is not mistaken for done

**The declared SECONDARY tenor (330–400 DTE) is NOT RUN and carries no verdict** — LEAPS cluster at
January so that window is empty on most observation dates. **The engine's own 45–75 DTE band is NOT
run**, by sec 4. **No cost-adjusted verdict is taken** — the verdict is on the FULL QUOTED SPREAD,
the conservative side, and the `O18` rho leg ships beside it **LABELLED AN EXTRAPOLATION** because
rho was measured on 35-delta ~60-DTE CALLS and this book is PUTS at 150–210 DTE. **`MA28` is not
weakened and not reopened** — this is a statement about whether the crash is PURCHASABLE, not about
whether the flag is real. **`MB8` is not reopened**; O-1 is on the PANEL and no arm path reads a
book file, pinned by test. **The 5.89% coverage ceiling is a property of the harvest freeze's
targeting, not of options data** — a deeper pull would move it, and that is a data purchase, not an
analysis.

**Expectations: the register put the kill firing at 15%, UNDERPOWERED at 35%, NULL at 35% and
separation at 15%. The kill did not fire and the verdict is UNDERPOWERED — the modal branch, and
the one whose cause (coverage) I estimated 17x wrong.** 34 tests, exit 0.
`scripts/o1_kill.py`, `scripts/o1_arm.py`;
...
**ZERO TRIALS.** No meter read, no verdict, `by_domain` untouched by this lane.
## 78. W-14 — CBOE OPEN-CLOSE: **REJECTED AT THE CENSUS GATE. The product is not on this grant,
under any name, in any of 221 libraries** (2026-08-25)

Executor's verdict on the Frontier Scout's `PREREG_DRAFT_w14_cboe_openclose.md` (2026-08-24).
**NO REGISTER WAS COMMITTED, NO ARM RAN, ZERO TRIALS** — options stays **308**, equity 242, infra
20, `by_domain` bit-identical across the log append while `rows_fixed_not_counted` rises **77 →
78**, the proof the row was seen and correctly excluded. Full reasoning in
`W14_EXECUTOR_VERDICT.md`. **The draft is REJECTED AS WRITTEN and its reasoning is largely
ACCEPTED — those are different things and both are recorded.**

* **THE DRAFT'S CENTRAL PREMISE IS FALSE ON THIS GRANT.** It reads *"`D4` priced this data at
  $500/mo and declined it; WRDS makes it free."* **WRDS does not carry it at all.** Its own **K3,
  the identification kill, fires** — and the scout called that in advance: *"This is the one most
  likely to fire; it is written first for that reason."*
* **WHY THE EXISTING CENSUS COULD NOT ANSWER IT, WHICH IS THAT CENSUS'S OWN LESSON ONE LEVEL
  DOWN.** The draft is census-gated (*"nothing below runs until `WRDS_CENSUS.md` confirms the
  product, fields and span"*), and `WRDS_CENSUS.md` neither confirms nor denies it: it probed the
  OptionMetrics-**replacement** shape — `optprice_2010`, `optprice_2016`, `ivlisted_2010`,
  `eqmaster`, `optcontract`, `wrds_eq_opt_merged` — and found every one denied. **It never probed
  open-close.** A denial of six optprice-shaped tables is evidence about the `cboe` grant and is
  **not a measurement of this product**: *"a census that probes the names in a brief measures the
  brief."*
* **MEASURED, READ-ONLY AND SCHEMA-ONLY, NO LICENSED ROW MATERIALISED. (a) The product does not
  exist here** — account-wide search for `opencl`/`open_close`/`openclose` returns **0
  candidates**, and enumerating all 90 tables of `cboe` shows what the library is: the
  **IvyDB/OptionMetrics-lineage schema** (`optprice_1998…2026`, `ivlisted_1998…2026`,
  `optcontract`, `optdeliv`, `eqmaster`, `ivborrowrate`). **(b) The only options volume available
  carries no origin** — `cboe_sample.optprice` reads at 12 columns with `volume` and `openint` as
  **totals**, which is K3's exact failure condition; `open_` there is the session's opening
  **price**, a name that reads as a hit to a grep and is not one. **(c) No table anywhere splits
  it** — a column search across all 221 libraries returns 4,863 columns over 2,719 tables, and the
  **six** carrying both a customer-ish and a firm-ish column are all **Bureau van Dijk corporate
  registries** (`main_customers` against `confirmation_dates`), false positives on the substring
  `firm`; a separate search for columns naming opening volume returns **3, all in
  `audit.feed59_bank_holding_company`**. **(d) And the `cboe` production tables are denied anyway**,
  re-confirmed rather than quoted. **Two independent reasons, and the first is sufficient.**
* **A CORRECTION TO `WRDS_CENSUS.md`, FOUND ON THE WAY AND REPORTED RATHER THAN EDITED.** It says
  *"Searched all 221 visible library names: **Intraday Indicators by WRDS** … not present …
  the honest state is ABSENT-ON-THIS-LOGIN rather than proven unavailable."* **It IS present —
  as TABLES, not a library: `taqm_2003.wrds_iid_2003` … onward plus
  `form_metadata.wrdsapps_all_iid_ms_orderflow`, 94 tables carrying a `retail` column, 13 each
  (`buyvol_retail`, `bs_ratio_retail_vol`, `buynumtrades_retail` — the Boehmer–Jones–Zhang–Zhang
  shape) — and it returns `permission denied`, which is the STRONGER evidence that census said it
  lacked.** Its conclusion stands and its reason changes; the failure mode is its own, **searching
  LIBRARY names for a product that lives in TABLE names**. `B13`/`S7` are untouched. **Not edited —
  the data lane owns that file**, and a factual correction to a shared census other registers gate
  on should be applied by its owner. **AND IT SHARPENS W-14's EPITAPH: a retail identifier does
  exist on this account. It is on the EQUITY tape, not the options tape, and it is denied.**
* **THE GRAVEYARD ARGUMENTS, AND THREE OF FOUR SURVIVE THE REJECTION.** **`MB15`:** its named
  successor axis is **condition + size** (the OPRA single-leg-auction flag plus small size), a
  **trade-level inference from our own tape about whether a print LOOKS retail**; open/close is a
  **venue-published aggregate of position-opening INTENT**. One is an inference we make, the other
  a fact the venue publishes — **a real distinction, and it does not save the item, because the
  published fact is not on this grant.** **The comparison cuts both ways:** `MB15`'s successor
  **is** buildable today and inherits the **alert-days-only** conditioning defect W-14 was invented
  to escape, so neither route currently gives an unconditioned options retail identifier.
  **`MB16`/`O14`:** the conditioning critique **survives intact** — all six null flow features were
  computed on the alert-day cache, *"selected toward the retail-heavy tail… does not generalise to
  ordinary days"*, so **a full-market unconditioned test of flow has never been run on this
  project** and the six NULLs do not settle it. That is the draft's strongest sentence and it is
  preserved for a successor. **`R2`:** accepted — a full-market flow signal would be a signal for a
  book that does not yet exist. **`MB12`:** correctly declined as a motivation; the draft leans on
  a mechanism rather than on novelty.
* **POWER AT BOTH VOCABULARIES BEFORE ANY FLOOR — AND IT EXPOSES A DEFECT INDEPENDENT OF THE
  ...
  multipliers **4.2253** and **4.1533**, i.e. **1.2481×** and **1.2535×** the 50%-power figure.
  **The draft's §4 fixes the number to beat as `MB16`'s SE 0.04817 → a 50%-power MDE of +9.64pp.
  That is `2.0 × SE` — the RETIRED 2.0 convention.** At this project's own hurdle the same SE gives
  **+16.31pp at 50% power and +20.35pp at 80%**, and **power against `MB16`'s own observed 8.35pp
  is 4.93%**. So the reference design is **worse than the draft says by roughly a factor of two**,
  and a successor inheriting it would set its floor at half the required size — `MB22`'s vocabulary
  correction landing on a live draft, after `S19`, `V2G` and `V6`. **What a successor would need:**
  on the draft's own premise that a full-market panel shrinks the per-month SE (against the alert
  cache's **median of 2 names per date**), detecting a **3.00pp** long-short effect at **80%
  power** takes **≈5,293 months** at `MB16`'s SE, **≈1,323** at SE/2, **≈588** at SE/3 and **≈212
  months — 17.7 years — at SE/5**. **Even a 25-fold larger cross-section per month needs nearly two
  decades of full-market data**, and that is knowable now, before anyone buys anything.
* **RE-OPEN CONDITIONS, CHECKABLE RATHER THAN ASPIRATIONAL.** (1) **A grant that reads the
  product** — the Cboe **Open-Close Volume Summary** is sold through DataShop/LiveVol and is not
  the WRDS `cboe` library, so **`D4`'s $500/mo purchase question is UNDISSOLVED**, which is the one
  thing the draft asserts that measures false. (2) **`taqm_YYYY.wrds_iid_YYYY` becoming readable**,
  giving an unconditioned **equity** retail identifier — not this hypothesis but the nearest live
  one, and a **subscription question for Don**, the same class as the `cboe` page-versus-grant
  disagreement that census already routed to him. (3) **The `MB15` condition+size successor**,
  buildable today and inheriting the conditioning defect, so it answers a **narrower** question and
  must say so. **None is proposed here**; each needs its own blind register and its own trials.
* **WHAT THIS DOES NOT SAY: it is NOT a finding that retail opening flow carries no information.**
  No arm ran; the hypothesis is **untested, not rejected**, and the literature's mechanism claim is
  untouched. Nor is it a finding about `MB16`'s six NULLs — that conditioning critique stands and
  is strengthened by being written down. Nor is it a claim that WRDS lacks retail-flow data: **it
  has it, for equities, and cannot read it.** **The counter question the draft flagged for the
  executor — equity or options — is MOOT and deliberately LEFT UNDECIDED**, since deciding it would
  imply an arm that will not run. **16 tests, exit 0; 12 of them run in CI and 4 skip loudly, and
