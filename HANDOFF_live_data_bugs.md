...
## Part 18 — FIDELITY-2: BOTH FAILED THEMES REBUILT AND RESTORED (2026-08-11, greeks lane)

`institutional` (+0.1706) and `insider` (+0.3596) failed the fidelity gate at 0.60. Rebuilt to the
panel's own definitions and re-scored against **the same bar, not re-derived**:

| theme | ρ before | ρ after | Δ | n | coverage | verdict |
|---|---|---|---|---|---|---|
| **institutional** | +0.1706 | **+0.9190** | **+0.7484** | 382 | 0.818 | **RESTORES** |
| **insider** | +0.3596 | **+0.8726** | **+0.5130** | 328 | 0.682 | **RESTORES** |

**Nothing closes UNRESTORABLE**, so no D-series data-purchase item is generated by this task.

`PREREG_fidelity2_rebuild.md`, committed **alone at `ef765fc`**, before any rebuild code existed.

### 18.1 THE DIAGNOSES — both were my construction, not the source

**`institutional`: the formula matched and both inputs diverged.**

* **DOLLARS vs SHARES, and I chose shares deliberately.** The panel's `_inst_accum`
  (`fundamental_panel.py:738`) reads `_f(r, "totalvalue", "value", ...)` — dollars first. My build
  used shares, and `tests/test_live_theme_sources.py` pins that choice with the reasoning
  *"Otherwise a quarter's price move would make this a momentum signal in a 13F coat."*
  **That argument is sound and it is exactly why the column was not the panel's:** the panel's
  quantity deliberately CONTAINS the price move. I built a better signal and a different one.
* **The quarters were two periods apart.** The panel at 2026-01-28 uses **2025-09-30 vs
  2025-06-30** at its 45-day lag; I used 2026-03-31 vs 2025-12-31. No overlap at all.

**`insider`: a different estimator, plus a fabricated neutral.**

| | panel | live scraper |
|---|---|---|
| magnitude | `Σ signed value (USD)` | `Σ code_weight × role_mult × **√value**` |
| cluster | `min(10, 2 × positive TRANSACTIONS)` | `min(10, 3 × (buying FILINGS − 1))` |
| empty window | **`None`** | **`50.0`** |

The square root is monotone; **the code and role weighting is not**, so it moves ranks. And the
fabricated neutral put **179 of 500 names in one tie block** against panel values that are absent
entirely — in this window, 159 names legitimately have no transaction and now correctly return
`None`.

### 18.2 THREE BUGS FOUND DURING THE REBUILD, all before they could corrupt a number

1. **V2G's dataset reader assumed a flat zip.** SEC packs `01sep2025-30nov2025` flat and
   `01jun2025-31aug2025` nested under a directory; opening by bare name raised
   `KeyError: There is no item named 'SUBMISSION.tsv'` and killed the run. It never bit V2G
   because both quarters it happened to need are flat. **A reader that works on the two archives
   you first tried is not a reader that works on the format.** Fixed with a member resolver.
2. **My aggregation stored the wrong level.** `aggregate_13f` returns a `{period, shape, cusips}`
   wrapper; I saved the wrapper, so it printed **"3 CUSIPs"** — literally its three keys — against
   V2G's 22,000. **The absurd number is the only reason it was caught** before the join silently
   produced an empty column and the gate scored noise.
3. **The live Form 4 parser exposes only UNSIGNED values.** `_parse_form4` returns
   `value_usd = shares × price`, both positive, and recovers direction through `CODE_WEIGHTS`.
   Summing those would have made `net` a **gross turnover** figure — a third distinct quantity.
   A signed parser reading `transactionAcquiredDisposedCode` was written and unit-checked
   (+1000 buy, −500 sale) before the crawl ran.

### 18.3 THE REBUILD REMOVED THE CONFOUND THE FIRST GATE CARRIED

`PREREG_theme_restoration.md` §2.3 warned the first gate was *conservative for a pass and
ambiguous for a fail* because the windows differed. Both themes were re-measured on **the panel's
own window** — the 2025-09-30/2025-06-30 quarters and a date-aligned Form 4 crawl over
2025-10-30 → 2026-01-28 (**5,639 documents, 500 names, 499 of 500 cached submission indexes
already reaching back far enough**). So these ρ are source-to-source comparisons, not
source-plus-drift.

### 18.4 WHAT SHIPS, AND HOW

**The gate proved the ESTIMATOR, not the window.** Production applies the same estimator to the
CURRENT window — the latest two complete 13F periods and the trailing 90 days — exactly as the
panel applied it to whatever was current at each of its own dates. A separate production crawl
(**6,133 documents**) built today's cache.

`valuation/screener/live_themes.py` reads a pre-built cache and **makes no network calls**:
building 5,600 SEC requests into a scan would put them on the critical path. It **fails to None**
on every path — missing file, stale file, unknown ticker — which `factors.py` treats as a neutral
factor and `composite` renormalises away, i.e. the pre-restoration behaviour. **Staleness is
reported rather than tolerated**: `status()` carries the build date and refuses a cache older than
120 days, because 13F periods roll quarterly.

Measured end to end on 60 names: **`institutional` 0 → 26 non-null with 26 distinct values**, and
**`insider` 1 → 20 distinct values** (it was the constant `0.0` that `factors.py:284` writes when
`insider_score` is absent — which is precisely why a "100% covered" column carried no information).
Coverage on the served universe: `inst_accum` 82.2%, `sm_breadth` 82.2%, `insider_score` 72.6%.

### 18.5 THE VINTAGE — AMENDED, NOT RE-OPENED, AND THE RULE WAS REGISTERED FIRST

The registers were **silent** on staged restoration — no "staged", no "partial restoration", no
"amend a vintage" in `PREREG_v1`, the contract or `CLAUDE.md`. So §4 of `ef765fc` registered the
interpretation **before any FIDELITY-2 number existed**:

> An adopted change made while the current vintage has accrued **ZERO complete days** AMENDS that
> vintage instead of opening the next.

Why it is faithful rather than convenient: **Rule 6 protects a clock and there was no clock to
protect** (vintage 3 opened the same morning); the shadow's predecessor stays **vintage 2**, the
four-theme book the comparison is actually about, where opening vintage 4 would shadow a book that
never ran a single day; and **it cannot become a loophole because it is bounded by a fact** — one
accrued day and Amendment 1's ordinary rule resumes.

**Consequences, all verified:**

* Vintage 3's opening date is **unchanged at 2026-08-11**; the gate stays **2027-02-11** and the
  verdict **2031-08-11**. **The clock was NOT reset a second time.**
* Vintage 3's `themes_scored_live` goes from five to **all seven weighted themes**.
* **Vintage 3's `params_id` DOES move**, `441531f1de2b` → `24878e43a1e3`, because the model
  genuinely changed. Recorded openly here because Part 17 published the old one.
* **Vintage 2's pin is untouched at `0060c5ef3dda`.**
* No fourth vintage. One shadow pair, still live 3 / shadow 2.

### 18.6 WHAT I DID NOT DO

* **Did not re-derive the bar.** It is imported from `theme_restoration.py`; §3 of the register
  forbids recomputing it, and it stayed 0.60.
* **Did not change the live insider SCORER.** `screener/insider.py` keeps its code/role-weighted
  statistic for its own surfaces; the panel-matching statistic is computed in the cache builder.
  Changing the shipped scorer would be a separate adoption with its own gate.
* **Did not retune anything.** Both themes enter at their deployed 0.125.
* **Did not build an automated refresh.** The cache is built by an explicit command; a scan with
  no cache degrades to the pre-restoration behaviour rather than failing. **Scheduling that build
  is the open item this leaves.**

### 18.7 EXPECTATIONS, SCORED

| prediction | odds | outcome |
|---|---|---|
| `institutional` passes | 55/45 | **RIGHT** (+0.9190) |
| `insider` passes | 35/65 | **WRONG — it passed** (+0.8726) |
| at least one passes | 65/35 | **RIGHT** |

The one I got wrong is the one I was most confident about in the negative direction — the fourth
time this project's directional calls have failed in a row where the register said they would.

**Zero trial cost; equity `N` unchanged.** Artifact `data/free_analysis/FIDELITY2.json` (under
`data/`, never committed). **28 tests** in `tests/test_theme_restoration.py`.

**BUGS FOUND (Part 18)**

1. **The 13F dataset reader assumed a flat archive** and died on a nested one. FIXED.
2. **The aligned aggregation stored a wrapper instead of the CUSIP map** — caught only because
   "3 CUSIPs" is absurd. FIXED.
