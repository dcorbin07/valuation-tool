...
| O16 | O | Is term_slope a front-IV level? | BLOCKED | INCONCLUSIVE |  | HANDOFF_optionsbot.md | 2026-08-07 | human | STOPPED AT ITS OWN PRE-REGISTERED REPRODUCTION GATE - no verdict on the hypothesis, deliberately. Recomputed term_slope matched the banked book on 86.435% of 3,885 rows against a required 99%. THE GATE FOUND AN INFRASTRUCTURE DEFECT, WHICH IS THE REAL OUTPUT: data/options is a LIVE store and 19.5% of the alerts ticker-year chain files were re-mined after state_r2_corrected.pkl was banked on 2026-08-05. Rows whose chain is untouched reproduce at 100.00% (3,127 of 3,127, zero exceptions); re-mined rows reproduce at 30.47%. The authoritative options book is therefore NOT reproducible from the current chain store, and any recompute-based audit of it silently disagrees on ~13.6% of rows. EXPLORATORY only, NO verdict, on the 3,358 exactly-reproducing rows: the committed rule WOULD have said IS DISTINCT (Spearman -0.5405 vs the 0.60 bar), and the residual of term_slope on atm_front predicts BETTER than raw term_slope while atm_front alone predicts nothing. Re-run needs pinned chains. |
