...
CIK_OVERRIDES = {
    ...
    "MRVL": [1058057, 1835632],     # Marvell Technology Group Ltd -> Marvell Technology Inc
                                    #   (23 + 21, clean handover at 2021-03/2021-06)
    "APO": [1411494, 1858681],      # Apollo Asset Management -> Apollo Global Management
                                    #   (31 + 17, OVERLAPPING 2022-05..2023-08 — see
                                    #   _dedupe_overlapping_registrants)
}
...
def _dedupe_overlapping_registrants(ticker: str, listed: list) -> list:
    """One filing per (form, fiscal period) when two CIKs cover the same quarters.

    A reorganization is not always a clean handover. Apollo's predecessor (Apollo Asset
    Management) kept filing until 2023-08 while the successor started in 2022-05, so five
    quarters exist TWICE — two different documents for the same period from two registrants.
    Left alone, `find_prior` can pair a successor 10-Q against the predecessor's 10-Q from a
    year earlier and read the change of registrant as the company rewriting its filing.

    Rule: keep the filing from whichever CIK is still filing latest — the surviving entity,
    which is the one shareholders actually hold.
    """
    last_by_cik: dict = {}
    for f in listed:
        c = f.get("cik")
        last_by_cik[c] = max(last_by_cik.get(c, ""), f["filing_date"])
    best: dict = {}
    for f in listed:
        key = (f["form"], f.get("report_date") or f["filing_date"])
        cur = best.get(key)
        if cur is None or (last_by_cik.get(f.get("cik"), ""), f["filing_date"]) > \
                          (last_by_cik.get(cur.get("cik"), ""), cur["filing_date"]):
            best[key] = f
    out = sorted(best.values(), key=lambda x: (x["filing_date"], x["accession"]))
    if len(out) < len(listed):
        _log(f"{ticker}: {len(listed) - len(out)} duplicate period(s) across registrants dropped")
    return out
...
def build_ticker_cache(ticker: str, cik, out_dir: str, limiter: RateLimiter,
    ...
    if len(ciks) > 1:
        listed = _dedupe_overlapping_registrants(ticker, listed)
...
def coverage_report(cached: dict, rows: list, requested: list, unmapped: list,
    ...
    return {
        ...
        # A 1,100-word "10-K" is a stub — a trust or subsidiary registrant filing boilerplate,
        # not a business description. It scores ~1.0 against last year's copy of the same
        # boilerplate, i.e. maximally "lazy" for a reason that has nothing to do with the
        # paper's mechanism. Reported so the IC test can filter on n_words.
        "rows_from_stub_filings_under_2000_words":
            sum(1 for r in rows if (r.get("n_words") or 0) < 2000),
...
def _short_dir(p: str) -> str:
    """`.../valuation-tool/data/filings` -> `data/filings`. A committed report should not
    carry one machine's absolute paths."""
    parts = os.path.normpath(p).replace(os.sep, "/").split("/")
...
def write_report_md(cov: dict, path: str, out_dir: str = DEFAULT_OUT_DIR) -> str:
    ...
    L = [
        ...
        f"`{_short_dir(out_dir)}/lazy_prices.csv` and is **gitignored**, so this file "
    ...
    L += [
        ...
        f"5. **Stub filings.** {cov.get('rows_from_stub_filings_under_2000_words', 0)} rows come "
        "from documents under 2,000 words — trust/subsidiary registrants filing boilerplate, "
        "which scores ~1.0 against last year's copy of the same boilerplate and so reads as "
        "maximally 'lazy' for a reason unrelated to the paper's mechanism. `n_words` is on "
        "every row; filter on it.",
        "6. **Amendments (10-K/A) are excluded** and rows built from a complete text "
