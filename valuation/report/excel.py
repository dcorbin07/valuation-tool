...
def withheld_workbook_rows(result, computed_at=None) -> list:
    ...
    numbers, the workbook shows the entire projection that produced them, plus a 5x5
    sensitivity grid of per-share values and a WACC sheet — and every one of those cells is a
    formula, so a "blanked" summary cell would recalculate the withheld figure the moment the
    file opened. So the refusal workbook is not the model with holes in it: the model sheets
    are not built at all. One sheet, text and the reference price, no formulas.
    """
    cd = result.company
    ...
    return [
        ...
        ("blank", "", None),
        ("wrap", "The model produced a figure, checked it against the market price and "
                 "refused to publish it. This workbook therefore contains no forecast, no "
                 "valuation bridge and no sensitivity grid: those are the same valuation in "
                 "another shape, and a formula-driven copy of them would simply recompute the "
                 "withheld number when you opened the file.", None),
        ("blank", "", None),
        ("head", "What is shown", None),
        ("kv", "Reference price", cd.price if cd.price else None),
        ...
        ("blank", "", None),
        ("note", "Educational tool, not investment advice. Generated by Valquo.", None),
    ]
...
def _build_withheld_workbook(result, path: str, computed_at=None) -> str:
    ...
    for kind, text, val in withheld_workbook_rows(result, computed_at):
        if kind == "blank":
            ...
            continue
        if kind == "title":
            _c(ws, f"A{r}", text, TITLE)
        elif kind == "head":
            _c(ws, f"A{r}", text, BOLD, fill=SECT_FILL)
            ws[f"B{r}"].fill = SECT_FILL
        elif kind == "kv":
            _c(ws, f"A{r}", text)
            if val is None:
                _c(ws, f"B{r}", "n/a")
            elif isinstance(val, (int, float)):
                _c(ws, f"B{r}", val, BLUE, CUR2)
            else:
                _c(ws, f"B{r}", str(val), BOLD)
        elif kind == "wrap":
            c = _c(ws, f"A{r}", text)
            ...
            _c(ws, f"A{r}", text, Font(name="Arial", italic=True, size=9))
        r += 1
    wb.save(path)
    return path
