...
def withheld_pdf_lines(result, computed_at=None) -> list:
    ...
    derived from the withheld valuation appears in it. What it DOES contain is the reason —
    the guard's own sentence, which quotes the implausible figure as the evidence for
    withholding it. Returning an error instead would say the export is broken; the export is
    ...
    return [
        ...
        ("body", "This is not a formatting problem or a missing-data error. The model produced "
                 "a figure, checked it against the market price, and refused to publish it. "
                 "Everything downstream of that figure — the bear/base/bull cases, the "
                 "sensitivity grid, the Monte Carlo distribution, the implied values from "
                 "peer multiples and the opportunity score — is withheld with it, here and on "
                 "the site, because each is the same valuation in a different shape."),
        ("h2", "What is shown"),
        ("kv", ("Price", _money(cd.price))),
        ("kv", ("Fair value", "not published")),
        ("kv", ("Upside", "n/a")),
        ("kv", ("Opportunity score", "not rated")),
        ("kv", ("Regime", str(result.classification.regime))),
        ("kv", ("DCF reliability", str(result.classification.dcf_reliability))),
        ("h2", "What would change it"),
        ("body", "Most refusals are a currency or share-count mismatch between the statements "
                 "and the quoted price, which is a data problem rather than an opportunity. "
                 "When the inputs reconcile, the valuation publishes on its own — nothing has "
                 "to be switched on."),
        ("small", "Sources: " + "; ".join(cd.sources[:6])),
        ("small", "Educational tool, not investment advice. Generated by Valquo."),
    ]
...
def _build_withheld_pdf(result, path: str, computed_at=None) -> str:
    ...
    for kind, text in withheld_pdf_lines(result, computed_at):
        if kind == "kv":
            ...
            continue
        if kv:                      # flush the pending key/value block before the next heading
            E.append(_kv_table(kv)); kv = []
        if kind == "title":
            E.append(Paragraph(text, title))
        elif kind == "sub":
            E.append(Paragraph(text, small))
            E.append(Spacer(1, 6))
            E.append(HRFlowable(width="100%", thickness=1.4, color=AMBER, spaceAfter=8))
        elif kind == "h2":
    ...
    if kv:
        E.append(_kv_table(kv))
    doc.build(E)
    return path


def _kv_table(rows):
    t = Table(rows, colWidths=[1.9 * inch, 4.0 * inch])
