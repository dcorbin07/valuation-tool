...
- **THE LONG-TENOR FINANCING CASE IS `NO-FLIP`, AND "FINANCING IMPROVES WITH TENOR" IS REFUTED AT
  THE LEVEL OF THE RATE ITSELF — THE ROLL LEG COLLAPSES EXACTLY AS PREDICTED AND THE FINANCING LEG
  GETS WORSE (2026-08-25, `SC-3` / TIER-E-FIN).**
  `PREREG_sc3_tier_e_financing.md` committed **ALONE and BLIND at `293a1e7`**, markdown only, zero
  `.py`, 179 lines, a strict ancestor of every commit computing a cost verdict; **2 options trials
  booked at `1fd7f3d` BEFORE any runner existed, `N` 308 -> 310**. **ADOPTS NOTHING; `O11` GOVERNS
  and a cost curve licenses no trade.**
  * **WHY IT IS NOT A RE-RUN.** `DEEPITM-FIN` closed the question at 60-90 DTE at **`rf + 701.87`
    all-in**, and its **closing reading** is what was tested: the benefit *"REQUIRES rolling"*, so
    the shortest tenor is the **worst case** — *"only clearly positive at a tenor we do not own."*
    **`MB4` then closed the tenor question on OWNERSHIP rather than economics** (*"CLOSED as
    unownable — the owned cache is hard-capped at 200 DTE and the Tier E pull reaches 858 DTE only
    for 2016-2018"*), and **`S3-I5` removed exactly that ground.** `MB4` is obeyed: its re-open
    condition is a **number** (`rf + 702`, *"nobody should spend a trial re-deriving this"*), and
    the 60-90 DTE figure is **NOT re-derived**.
  * **COVERAGE MEASURED FIRST, ON THE POPULATION THE ARM WAS SCORED ON — `O-1`'s lesson applied
    rather than repeated.** On the **PINNED** Tier E harvest freeze, 2016-2018: **35,396,569 chain
    rows** in the 200-858 DTE band, **16,183,817** two-sided matched pairs, **2,352,345 SCOREABLE
    pairs over 411 names** and 754 dates, **408 clearing `DEEPITM-FIN`'s own `MIN_N` = 30** —
    against that study's **12,904 pairs over 185 names**. **THE FLOOR WAS REACHABLE AND WAS
    REACHED**: the register's floor is >= 100 names per stratum and the four strata carry
    **408 / 358 / 348 / 340**, clearing it by 3.4x to 4.1x.
  * **`NO-FLIP` IN ALL FOUR PRE-DECLARED STRATA, AND DECISIVE RATHER THAN A FAILURE TO DETECT.**
    All-in bps/yr on the **EXECUTABLE** leg: **915.4** [872.4, 953.1] at 200-300, **798.0** [765.9,
    860.7] at 300-450, **676.5** [641.8, 713.7] at 450-650, **757.9** [710.4, 794.6] at 650-858.
    **Every interval sits far ABOVE Robinhood Gold's `rf + 420`** — the option route is **DEARER**,
    not merely un-provably cheaper. Realised **MDE80 247.0 / 282.5 / 272.1 / 285.4 bps**.
  * **THE FINDING: THE ROLL LEG COLLAPSES AND THE FINANCING LEG DOES NOT.** Roll runs **434.6 ->
    326.4 -> 240.8 -> 250.4** bps as rolls/yr falls **1.62 -> 0.97 -> 0.67 -> 0.49**, so that half
    of the mechanism is real and confirmed. **But the SPREAD-FREE MID financing excess RISES
    monotonically — 112.8 -> 125.9 -> 143.7 -> 162.4 bps** — and the executable leg is flat-to-
    rising at 472.9 -> 451.2 -> 426.4 -> 493.5. **So the claim is refuted at the level of the RATE
    ITSELF, not merely offset by costs.** Mechanism measured rather than asserted: the absolute
    half-spread on the call leg **more than doubles, $0.95 -> $2.05**, growing fast enough that the
    1/T amortisation cannot outrun it. **CONSEQUENCE: the all-in is NOT monotone in tenor** — it
    bottoms at 450-650 DTE and **rises again**.
  * **AND IT REFUTES `DEEPITM-FIN`'s CLOSING CONJECTURE.** That item read *"60-90 DTE, the SHORTEST
    tenor, is the worst case."* **Measured, 200-300 DTE is WORSE (915.4 against its 701.87), and
    the best long tenor merely TIES it — the 450-650 interval [641.8, 713.7] CONTAINS 701.87.** At
    its cheapest long tenor the option route costs **statistically the same** as at 60-90 DTE.
    **The tenor we did not own was not hiding a better case, and `MB4`'s closure survives on new
    evidence.**
  * **THE PRIOR WAS WRONG AND THE REGISTER NAMED THE EXACT FAILURE MODE IN ADVANCE.** Registered
    FLIPS-GOLD 45% / NO-FLIP 40% / FLIPS-BOTH 5% / UNDERPOWERED 10%. The §1 arithmetic — roll
    collapse implies all-in near `rf + 407` — **assumed financing would hold near 342. It rose.**
    §8 named that as the reason for the substantial NO-FLIP weight.
  * **A CAUTION STATED BEFORE THE RUN AND QUANTIFIED BY THE RESULT.** The single-name smoke test on
    **AAPL read 105.8 bps against a cross-name median of 676.5** in the same stratum — AAPL sits at
    the **1.1st percentile of names**. A test on the most liquid name would have read ~6x too cheap
    and produced a confident FLIPS-BOTH. **The per-name unit of independence exists for exactly
    this**; 2.35M pairs are ~408 names on overlapping contracts and the pair is never the unit.
  * **THE BINDING LIMITATION, DECLARED BEFORE THE RUN: Tier E reaches past 200 DTE for 2016-2018
    ONLY, and that window is a near-zero-`rf` regime.** `DEEPITM-FIN` found the option leg's cost
    stable across all five rate eras (616-754 bps) while the Gold spread swung 52-567 — **but that
    is evidence from a DIFFERENT tenor, and era-stability at long tenor is UNMEASURED and remains
    so.** A successor wanting era-robustness needs a Tier E pull outside 2016-18, which is a **data
    purchase rather than an analysis.**
  * **REPORTED OUTSIDE THIS LANE (`RUN_RULES` rule 3): `HANDOFF_optionsbot.md` now carries TWO
    `## 77.` sections** — this lane's `O-1` and another lane's — **on top of the two `## 73.`
    sections already reported at `W-14`. Three duplicate section numbers from four lanes.** Mine
    landed first in both cases and **neither is renumbered**, since renumbering another lane's
    section breaks anyone citing it. **NOT DONE: no variant, no grid, no parameter the register
    does not name; all four declared strata reported (quoting a subset is a void condition); the
    60-90 DTE figure NOT re-derived; no return claim made anywhere and none reachable, pinned by
    test. 21 tests, exit 0 — 16 in CI, 5 skipping loudly.** `scripts/sc3_coverage.py`,
    `scripts/sc3_arm.py`; `data/free_analysis/SC3_COVERAGE.json`, `SC3_ARM.json`, `SC3_PAIRS.pkl`;
    `HANDOFF_optionsbot.md` 79.
...
- **THE SECTOR IS NOT A DIAL, IT IS A METHOD SWITCH — WIRING S25's DATED MAP INTO THE
  POINT-IN-TIME VALUATION MOVES THE FAIR VALUE ON *EVERY* ROW IT TOUCHES, AND FLIPS THE
  VALUATION REGIME ON 57% OF THEM (2026-08-26, `S25-REPAIR`).** `S25` measured the repair on
  the panel's *inputs* and reported a median target-margin delta of **+0.03**, because
  `ev_ebitda_used` is a boolean flag and the fair value could not be recomputed. Re-valued
  properly it is far larger. **ZERO TRIALS, `FIXED`-class** — `by_domain` bit-identical across
  the log append (equity **243**, options **310**, infra **20**) while `rows_fixed_not_counted`
  rises **79 -> 80**. **ADOPTS NOTHING** — `CONFIG` untouched, live path untouched, banked panel
  not overwritten, and `sector_map=None` leaves the builder bit-identical.
  * **THE CONTROL IS EXACT AND IT IS WHAT LICENSES EVERY FIGURE: the fresh build reproduces the
    banked `panel_s23_fairvalue.pkl` at max absolute fair-value deviation 0.000e+00 across all
    108,241 rows**, regime identical on 108,241 and sector identical on 108,241 — so the arms
    are measured against exactly the object the record describes. The count is GATED, because
    `MB21`'s `C1` once scored a perfect zero on an empty frame by comparing nothing.
  * **ONE BUILD, THREE SCORINGS, A PROVABLY IDENTICAL ROW SET.** Base and both arms are computed
    in one pass over the same rows from the same `CompanyData`, differing in the **sector string
    alone**. Three builds would have let the row set drift and confounded the repair with
    whatever else moved.
  * **REPAIR-A (CHANGE-ONLY, the register's PRIMARY): 4,982 rows move sector (4.60%), and ALL
    4,946 comparable rows change fair value — not one is unmoved.** Median **-13.2%** relative
    and **-10.1pp of price**; **4,250 (86%) move by >=10%**; **2,832 (57%) change REGIME** and
    2,893 change METHOD; **direction 3,180 DOWN against 1,766 up**, so it is a correction rather
    than a systematic upgrade; 22 rows the repair could not value, counted and not imputed.
  * **THE MECHANISM, AND IT IS WHY THE INPUT MEASUREMENT UNDERSTATED IT: the sector also selects
    the valuation REGIME, and `financial` is a DIFFERENT METHOD** — justified P/B from ROE rather
    than DCF plus multiples. Top transitions: **`mature` -> `financial` 1,064**, `cyclical` ->
    `mature` 425, `growth` -> `financial` 358.
  * **A NAMED, CHECKABLE EXAMPLE: Visa on 2009-01-15.** The panel calls it Financial Services
    today; **GICS classified it Information Technology until the 2023 payment-processor
    reclassification**, so the regime goes `financial` -> `mature` and the fair value **1.25 ->
    8.16**. Alphabet the same date: Communication Services -> Technology, 20.85 -> 25.49. Real
    GICS moves, not artefacts.
  * **THE NUMBER MOST LIKELY TO BE MISQUOTED IS THE MEAN, AND BOTH MEANS ARE OUTLIER-DRIVEN IN
    OPPOSITE DIRECTIONS.** The relative mean is **+32.7%** against a median of -13.2%, because
    its denominator is the base fair value and the `financial` regime can legitimately return
    **0.027** on a real name — switching out then reads as **+109x**, a property of the
    DENOMINATOR and not of the repair. The price-denominated mean is -0.84 against a median of
    -0.10. **Quote the MEDIANS: they agree with each other (-13.2% and -10.1pp) and the means do
    not.**
  * **REPAIR-B (FULL) IS 13,799 rows (12.75%), 6,167 changing regime, median -3.6%** — sensitivity
    only, **CONFOUNDED by construction**, and quotable only beside the 11.37% taxonomy
    disagreement, which now ships inside the artifact so a reader of one file cannot breach the
    void condition. **One asymmetry: under A ZERO rows moved sector without moving fair value;
    under B 486 did** — B includes pure taxonomy relabelling that can land on the same margin.
  * **A SCOPE CORRECTION THAT RUNS IN THE PROJECT'S FAVOUR, AND IT CHANGES WHAT ADOPTION MEANS:
    THE REPAIRED PATH IS RESEARCH-ONLY.** Measured — `build_valuation_panel` and `pit_company`
    have **ZERO importers under `valuation/web/`, `valuation/saas/` and `valuation/screener/`**.
    The live site values through `pipeline.value_from_company` with today's sector, **and for a
    valuation dated TODAY, today's sector IS the point-in-time sector.** The look-ahead exists
    only where a HISTORICAL date is scored against a PRESENT classification.
  * **SO ON THE VINTAGE RULE'S OWN WORDING THIS IS PROBABLY NOT A VINTAGE EVENT** — that rule
    defines *"adopted"* as **"ships in the live scoring path"**, and this does not ship there,
    does not change the hot list, the Index, or any number a user receives. **THE
    CLASSIFICATION IS DON'S AND IS NOT MADE HERE**; it is flagged because acting on the
    assumption would reset a clock for nothing. **What an adoption would really cost is
    RE-DERIVATION:** the banked panel is read by `scripts/exit_rule.py` (`S23`),
    `mb18_expectations_gap.py` (`MB18`) and `ma_final_batch_measure.py` (`MA26-C`), with `S10`'s
    band panel a sibling build. **And if it were treated as one, the open vintage is 4, opened
    2026-08-13 on the S14 band adoption with 13 calendar days accrued and a 2031-08-13 verdict
    horizon** — derived, never quoted.
  * **THE FLEET DOES NOT ATTACH TO THE VINTAGE, AND THAT CORRECTS THE BRIEF.** `fleet.py` and
    `fleet_books.py` contain the string "vintage" **zero times**, and a book's clock is
    `horizon_reached = n >= fills_needed`, **a FILL COUNT declared per book** rather than a
    calendar clock. **The FORWARD TRACK does attach** (`track_meter.VINTAGES`, and the contract's
    *"the operational gate and meter attach to the current vintage"*). A book whose entry rule
    read a changed score would face a CONTAMINATION question, not a clock reset.
  * **A REFUSAL KEEPS THE PANEL'S OWN SECTOR AND NEVER BLANKS IT, in both arms.** Both engine
    dicts fail open, so blanking an unknown sector does not abstain — **it hands the row the
    middle of a 2.70x range.** All four refusal states keep the base sector and the state travels
    back in `sector_state` so refusals are COUNTED: **104,097 `OK`, 4,144 `NOT_COVERED`, zero
    `UNMAPPED`.** The map is **duck-typed in, never imported** — an engine module importing
    `valuation/edge/` would put a study-side dependency on the live valuation path — pinned by an
    AST test with a positive control.
  * **NOT DONE: nothing adopted**, the S25 exposure pin stays green and is correct;
    **`SECTOR-NEUTRAL-B6` NOT re-opened**; **no landed figure re-derived** (S23, MB18, MA26-C
    still rest on the banked panel); `S10`'s band panel not rebuilt; pre-1999 rows still
    unrepairable. **14 tests, zero skips; 4 of 4 mutations caught with sources restored
    byte-for-byte.** `valuation/engine/calibration.py` (`s25_repair_sectors`),
    `scripts/s25_repair_panel.py`; `data/free_analysis/S25_REPAIR_VALUED.json`,
    `panel_s25_repair.pkl`; `HANDOFF_edge_audit.md` S25-REPAIR.
- **THE POINT-IN-TIME SECTOR MAP EXISTS AT LAST, AND THE LOOK-AHEAD IT REPAIRS TOUCHES 4.60% OF
  THE BANKED VALUATION PANEL — BUT ITS CROSSWALK COSTS 11.37% DISAGREEMENT, WHICH IS LARGER THAN
  THE THING IT REPAIRS (2026-08-25, `S25`).** **THE FIRST PERMANENTLY-CLOSED LEDGER ROW WHOSE OWN
  NAMED EXIT CRITERION HAS BEEN MET** — `S25` closed `UNOBTAINABLE-WITHOUT-NEW-DATA` on
  2026-08-12 and wrote its route back down; `comp.co_hgic` is that route, dated.
  `PREREG_s25_sector_crosswalk.md` committed **ALONE at `928e46c`**, markdown only, zero `.py`,
  151 lines, a strict ancestor of every measurement commit, **BEFORE any crosswalk code existed**.
  **ZERO TRIALS, `FIXED`-class** — `by_domain` bit-identical across the log append (equity **243**,
  options **308**, infra **20**) while `rows_fixed_not_counted` rises **78 -> 79**. **ADOPTS
  NOTHING, WIRES NOTHING, RUNS NO RANKING ARM.**
  * **THE CROSSWALK IS A CHOICE AND ITS TARGET WAS FORCED RATHER THAN PREFERRED.** Eleven GICS
    sectors and eleven panel sectors admit many mappings; the shipped one is declared cell by cell
    with a reason per cell. The TARGET is not taste: `SECTOR_TARGET_MARGIN` and `SECTOR_MULTIPLES`
    are keyed on exactly eleven strings, so a mapping onto anything else would be unusable by the
    thing it exists to repair. `engine_sector_keys()` **IMPORTS** those keys rather than retyping
    them (`MA5`), and **zero GICS codes came back UNMAPPED on the real pull.**
  * **BOTH ENGINE DICTS FAIL OPEN, MEASURED NOT ASSUMED, AND THAT IS WHY `UNMAPPED` IS A NAMED
    STATE.** `SECTOR_TARGET_MARGIN.get(cd.sector, 0.12)` and `SECTOR_MULTIPLES.get(cd.sector,
    _DEFAULT)` — **so a crosswalk returning nothing is not neutral, it is a VOTE for 0.12 in the
    middle of a 2.70x range.**
  * **THE COST WAS DECLARED IN ADVANCE AND THEN MEASURED: 205 of 1,803 covered names disagree
    with the panel's own sector TODAY — 11.37%**, largest pair Technology (panel) against
    Communication Services (GICS). **The labels map 1:1; the MEMBERSHIP does not.** It is required
    output rather than a defect, **because without it a repair that changes a name's sector cannot
    be attributed** — the change is either look-ahead being fixed or the taxonomy switch.
  * **COVERAGE: 2,314 of 2,441 panel tickers (94.8%) — measured on the S23 VALUATION panel, which is NOT the 2,531-name metrics panel the register quotes the census 94.9% against; two nearly-equal percentages on different objects**, 3,853 dated spans, 992 names (42.9%)
    reclassified at least once** — which is what makes it a HISTORY rather than a snapshot wearing
    a date. **ZERO ambiguous tickers.** **AND 638 NAMES ARE NOT COVERED *TODAY*, WHICH IS CORRECT
    BEHAVIOUR RATHER THAN A HOLE:** their latest span carries an `indthru` in the past because they
    delisted, and **a lapsed classification is not evidence about today**, so `current()` refuses
    rather than carrying it forward. A date falling in a GAP between spans refuses the same way.
  * **THE LOOK-AHEAD REFUSAL IS THE PROPERTY THE MODULE EXISTS FOR: a date PREDATING a name's
    first classification returns `NOT_COVERED` and NEVER the first span.** Returning it would read
    as coverage, produce a plausible sector, and be undetectable downstream — the exact defect
    being removed, re-introduced by the thing removing it. Pinned with a **positive control** so
    the guard cannot pass by refusing everything.
  * **A REVISION IS NOT A CORPORATE EVENT, AND THE RULE NEEDS BOTH CONDITIONS.** 1,539
    transitions — **1,403 `FIRM_RECLASSIFICATION`, 74 Real Estate 2016, 62 Communication Services
    2018.** `classify_transition` requires the date window **AND** the destination code, because
    **either alone over-claims**: a window alone relabels every ordinary reclassification in that
    month, a code alone relabels every move into Real Estate for the next decade. Both one-sided
    cases ship as negative controls. **THE FLAG IS DIAGNOSTIC, NOT A FILTER** — for a VALUATION
    repair a revision really does change which margin a name is scored against, whatever caused
    it. `I-4`'s codes-34/35 sunset in a new table.
  * **A PREMISE CORRECTION THAT RUNS AGAINST THE BRIEF.** It cites 1,053 matched tickers mapping
    to more than one permno; **on the COMPUSTAT side that figure does not transfer — 77,411
    tickers map to exactly one gvkey and only FOUR map to two, none of them in our universe.**
    The 1,053 is a CRSP-side *permno* count. The `AMBIGUOUS_TICKER` refusal is implemented, tested
    and mutation-tested, and **never fires on the real data**; it ships anyway, because a refusal
    that costs nothing today is the one you want present the day the universe moves.
  * **THE HONEST LIMIT, IN THE ARTIFACT RATHER THAN IN A HANDOFF: `comp.security` CARRIES NO DATE
    COLUMNS**, so the ticker->gvkey link is itself a SNAPSHOT. The four-ticker figure bounds
    *simultaneous* ambiguity; **TEMPORAL reuse — a ticker that was company A in 2009 and is company
    B today — is NOT observable on this route at all.**
  * **THE REPAIR, ON 108,241 BANKED VALUATION ROWS: 104,097 (96.2%) resolve, and REPAIR-A moves an
    engine input on 4,982 rows — 4.60%.** 6,235 rows carry a GICS reclassification and **1,253 of
    them move NOTHING**, because GICS moved the name between two codes the crosswalk sends to the
    same panel string — **reported rather than folded into either count, since quoting 6,235
    overstates the repair by 25% and quoting 4,982 understates the reclassification count.**
    **1,178 of the 6,235 (18.9%) are TAXONOMY-REVISION driven**, so about a fifth of what this
    repair moves is an index provider's paperwork — moved *correctly*, because the margin really
    does change.
  * **THE NUMBER MOST LIKELY TO BE MISQUOTED IS THE MEAN.** Target-margin delta on changed rows:
    **median +0.03, range -0.17 to +0.17, mean -0.0015, and 2,151 rows move by 0.05 or more.**
    The near-zero mean means the reclassifications **point both ways**, NOT that the repair is
    small. Sector PE delta median -2, range -17 to +17. Legs the changed rows actually used:
    multiples 4,611, DCF 3,775, growth 3,741, **neither 371**.
  * **REPAIR-B (FULL) IS 13,799 ROWS (12.75%) AND CARRIES NO VERDICT** — it fixes look-ahead
    **and** switches taxonomy in one step, so it is confounded by construction and **may only be
    quoted beside the 11.37% disagreement that explains most of it.** A void condition of the
    register, not a preference.
  * **THE FAIR-VALUE CONSEQUENCE IS NOT MEASURED AND IS NOT REPORTED AS ZERO — and the reason was
    CHECKED rather than assumed.** I intended to price the comps leg from the banked panel;
    **`ev_ebitda_used` and `ev_sales_used` are BOOLEAN METHOD FLAGS, not the multiples**, so the
    multiple a row was scored against is unrecoverable and the leg cannot be recomputed without a
    valuation re-run. What ships is INPUT movement plus the leg census that bounds it.
  * **NOT DONE, named so it is not mistaken for done: `SECTOR-NEUTRAL-B6` IS NOT RE-OPENED.** It
    was REJECTED TWICE on measurement in both held-out directions and `S25` was only one of its
    two named routes back — **a dated map removes the DATA objection and does not touch the
    REJECTION**; re-opening needs its own blind register and its own trials, and is not proposed.
    **`calibration.py` IS DELIBERATELY UNCHANGED and the pin
    `test_s25_the_pit_valuation_still_reads_a_non_point_in_time_sector` STAYS GREEN, WHICH IS
    CORRECT** — the exposure is still live, and wiring the repair is a construction change with a
    real blast radius and Don's call. Also not done: **no fair value recomputed; the pre-1999 rows
    are NOT repairable at all** and no sector is invented for them (`S23`'s path reaches 1998);
    **`ggroup`/`gind`/`gsubind` are pulled but not exposed**; and temporal ticker reuse is
    unmeasured. **34 tests, zero skips; 8 of 8 mutations caught with sources restored
    byte-for-byte.** `valuation/edge/sector_map.py`, `scripts/s25_sector_map.py`;
    `data/free_analysis/S25_SECTOR_MAP.json`, `S25_REPAIR.json`; **raw GICS rows stay in
    `D:\wrds` and never reach the repo**; `HANDOFF_edge_audit.md` S25.
