...
REFUSAL.** `VALQUO_MASTER_AUDIT_4.md` **MB25** commissions TIDEMARK as a linked page at
`/tidemark`; **MB26** names the one thing that must not ship beside it. Both executed.
**Zero trials — no hypothesis, no threshold, no verdict against a bar.** `by_domain` is
unchanged across this session's work at equity **234**, options **302**, infra **15**, and no
published claim moves. Nothing under `.github/` was touched.

---

## 0. Premise checks, before any code

**(a) `VALQUO_MASTER_AUDIT_4.md` IS ON `main`, AND MY WORKTREE WAS 31 COMMITS BEHIND IT.** The
file was not in this worktree at all and was first read from the `audit4-frontier` worktree.
Before executing a word of it, the landed copy was extracted (`git show origin/main:…`) and
diffed against the frontier draft: **byte-identical, 1,514 lines both**. So the spec executed is
the spec that landed, verified rather than assumed. `origin/main` was then merged in (clean, 28
files, no conflicts) so the gate below describes the tree that will actually be merged.

**(b) MB25 NAMES THREE LICENCE-CONSTRAINED SOURCES AND ONLY TWO OF THEM EXIST HERE.** It says
*"Case-Shiller (S&P copyright, explicitly not redistributable), NAREIT (terms unverified) and
Ken French may not be displayed raw"*. Measured against TIDEMARK's anchor registry:
`us_housing_price_to_rent` reads Case-Shiller through `fred_CSUSHPINSA`, and
`reit_dividend_yield` / `reit_yield_spread` read `nareit_monthly`. **Ken French feeds no anchor
in TIDEMARK at all**, so that third constraint is real and vacuous here. Said plainly in the
module docstring, because *"we complied with three constraints"* reads stronger than *"two of
them applied"*.

**(c) THE LICENCE ARGUMENT IS TIDEMARK'S, NOT MINE, WHICH IS WHAT MAKES IT CHEAP.** Its
dashboard builder's own docstring carries the rule — *"No forward-return numbers. None."* — and
the two bands it publishes are `SE(p) = sqrt(p(1-p)/n_eff)` and `1.96/sqrt(episodes)`, neither of
which needs a licensed value. **Nothing had to be stripped**, which MB25 predicted and asked to
be cited as such.
...
| file | role |
|---|---|
| `valuation/web/tidemark_surface.py` (new) | owns every TIDEMARK string and every derived statistic |
| `valuation/web/templates/tidemark.html` (new) | render only — holds no TIDEMARK prose, pinned |
| `valuation/web/app.py` | the `/tidemark` route, on the `/methodology` pattern |
| `valuation/web/templates/_saas_base.html` | one footer link, so the page is reachable |
| `tests/test_tidemark_surface.py` (new) | 25 tests, 15/15 mutations caught |

**A LINKED PAGE, NOT A TAB, AND MB25 GIVES THE REASON MB26 THEN ENFORCES.** A tab implies the two
projects are one product, and a reader who averages a Valquo statistic with a TIDEMARK one has
been misled by the layout rather than by a sentence. `test_tidemark_is_a_linked_page_and_not_a_tab`
asserts the hot-list script does not reference the route.

**WHAT CROSSES THE BOUNDARY: nine fields, and MB25's allowlist is implemented literally** —
`market`, `anchor`, `percentile`, `episodes`, `n_eff`, `band_sigma`, `tier`, `refusal`, `as_of`.
Sixteen markets, and every row carries exactly those keys and nothing else.

**THE TABLE IS MACHINE-COPIED, NOT HAND-TYPED.** The derived figures were generated by running
TIDEMARK's own `collect()` and emitting the allowlisted fields as a Python literal, so a
transcription error is not one of the ways this can be wrong.

**THE PERCENTILE BAND IS RECOMPUTED RATHER THAN CARRIED, AND THAT TURNS A CONVENIENCE INTO A
CONTROL.** `percentile_se()` implements TIDEMARK's own formula from the stored percentile and
effective n. Measured against TIDEMARK's published standard errors on all 13 markets that have
one: **max absolute difference 0.000e+00 — bit-identical**. A wrong `n_eff` in the table would
have surfaced as a mismatched band rather than as a plausible one.

---

## 2. The copy, and what "verbatim" can honestly mean here

The captions ship on the `V3` / `hold_horizon.py` precedent: the module owns them, the template
renders them, and a test fails if the template holds one literally.

**THE HEADLINE IS THE FINDING, NOT A CAVEAT ATTACHED TO ONE:** *"The rotation question was asked.
It came back unanswerable."* It is rendered above every market reading, and
`test_the_unanswerable_finding_is_stated_as_the_headline_not_a_caveat` asserts that ordering —
a reading rendered above the ruling would invert the page.

**MB25's REQUIRED SENTENCE SHIPS IN TIDEMARK'S OWN WORDS**, because *"a dashboard shipped without
that sentence reads as a signal"*: **NO PHASE-2 QUESTION MAY BE ASKED ON THIS DATA** … *the
best-placed market has 82 of the 155 independent years required, and the worst has 25.* Its more
useful inverse ships beside it: the edge would have to run at an **IR of 0.41 to 0.74** before
this data could detect it, against the charter's own plausible range of **0.2 to 0.4**.

**THE POOLED EFFECTIVE n IS 3**, and the page says why: six separate cheap episodes between 2008
and 2016 are one macroeconomic event seen six times, so three markets pooled debounce to 3
against a naive sum of 10 — a **3.33x** co-movement discount on a common window of **24.7 years**.

**A LIMIT ON THE PIN, STATED RATHER THAN IMPLIED.** TIDEMARK is not in this repository, so no
test can diff these strings against its documents. What is enforced is the achievable half: every
caption is a **committed literal** (a reword shows in a diff — `MA13`'s idiom) and every caption
is asserted to **survive into the rendered page**. **Drift between these strings and TIDEMARK's
own wording can only be caught by a human re-reading both.** That is a real weakness of this pin
and it is written into the module docstring, not just here.

---

## 3. MB26, implemented rather than promised

Both denominators go on the page, each labelled with the project it belongs to, and no statistic
from one may appear inside a sentence about the other —
`test_no_combined_verdict_and_no_cross_project_sentence` splits the rendered prose into sentences
and asserts no TIDEMARK figure appears in a Valquo sentence.

**VALQUO'S DENOMINATOR IS DERIVED AT RENDER TIME, AND THIS SESSION IS THE ARGUMENT FOR THAT.**
MB25/MB26 quote **549 trials at 3.3031 (equity) / 3.3775 (options)**. Measured on the pre-merge
worktree the register read equity **232** (hurdle 3.3005) and options **297** (3.3745) — **both
audit figures stale**. Measured again after merging `main`, equity is **234**, whose hurdle is
**3.3031, exactly the audit's number**, while options is **302** at **3.3795**, still not the
audit's 3.3775. **The same pair of quoted constants was wrong, then half right, inside one
session.** A number typed onto this page would have been wrong for one of those two reads.
`test_valquo_denominator_is_derived_and_never_typed` fails if any of those literals is pasted
into the module body.

**TIDEMARK's 66 and 2.89 ARE literals**, because this repository cannot read that project at
runtime. `sqrt(2*ln 66) = 2.8947`, which TIDEMARK reports as `|t| ~ 2.89`; both were verified
against `TRIALS.md` rather than copied from the audit. The asymmetry — one derived, one literal —
is deliberate and is disclosed, and the literal carries TIDEMARK's vintage beside it.

---

## 4. The staleness gate

`index_track.gate_state()`'s shape: **every unrecognised outcome fails toward showing no number**.
Past `STALE_AFTER_DAYS` the page renders the vintage and a staleness note **instead of** every
reading — percentile, episodes, effective n, band **and the tier** all go `None`. The tier goes
too, deliberately: it is a verdict about how much a reading is worth, and leaving it standing
beside a suppressed reading would be a judgement with nothing under it.

**THE NUMBER IS 90 DAYS, PRE-COMMITTED, AND I AM NOT BLIND — disclosed on TIDEMARK's own
precedent of disclosing exactly this.** The vintage was `2026-08-17` and two days old when 90 was
chosen. It is derived from the data's frequency rather than from today's age: the anchors are
monthly series published with a one-month lag, so a vintage older than a quarter has missed at
least three observations that exist. It is a presentation choice and no verdict depends on it.

---

## 5. Four defects of my own, three of them caught by guards this project already had

**(a) I WROTE A FIFTH COPY OF `sqrt(2*ln N)`, WITH `MA5`'s WARNING IN VIEW.** `valquo_denominator()`
computed the hurdle inline. `MA5` established that this expression must be written **once**, in
`statistics.hlz_hurdle`, precisely because the same idea written four times is how a frozen `3.0`
survived in three of them — and
`test_ma5_there_is_exactly_one_sqrt_2_ln_N_in_the_shipped_package` **failed**, naming
`tidemark_surface.py` beside `statistics.py`. **The guard `MA5` built caught a fifth copy from a
lane that had just read the warning.** Now delegated; the reason is in the docstring so the next
author does not rediscover it.

**(b) MY ROUTE GUARD READ THE ROUTE'S DOCSTRING AS CODE — the `MA5`/`MA49(c)` defect, third
instance in this repository.** It grepped the route body for `"percentile"` and failed against a
**correct** tree, because the route's own docstring explains that the payload carries
percentiles. It now reads the **AST**, drops the docstring, and judges by literals: no numeric
constant, and no string constant other than the template name. That is also stricter than the
grep it replaced — and `test_the_route_guard_is_not_vacuous` proves it still bites, including
that a docstring mention no longer trips it.

**(c) THE SENTINEL'S PAGE-HALF WAS VACUOUS, AND ONLY THE VACUITY TEST FOUND IT.** MB25 is explicit
that an allowlist alone can pass by emitting nothing, so a sentinel positive control is required.
My first cut injected `123456.789` as an extra key and asserted it reached neither the payload nor
the page. The payload half is real. **The page half could never have failed** — the template
renders *named* fields, so an unknown key cannot reach the HTML by that route, and the assertion
was guarding nothing. The control now exercises each half where it can actually fire: the payload
half by widening the allowlist, the page half by putting the sentinel in `refusal`, **which is
the realistic leak path anyway — it is free prose, and free prose is where a level would arrive
if one ever did.**

**(d) I RAN A GATE AGAINST A BASE 31 COMMITS STALE.** It was stopped rather than reported, and the
only gate quoted below is the one on the merged tree. A suite count from a stale base is not
evidence about the tree being landed.

---

## 6. Verification

* **`tests/test_tidemark_surface.py`: 25 passed**, 25 definitions against 25 reachable tests —
  asserted by `test_no_test_in_this_file_is_shadowed_by_a_duplicate_name`, which exists because
  session 40 silently deleted a test by naming a new one the same thing and the suite stayed
  green throughout.
* **Mutations: 15 of 15 caught**, all sources restored byte-for-byte and re-verified, with the
  harness run under `-B` and a `__pycache__` purge because CPython validates bytecode on
  (mtime, size). They include: the payload no longer projecting onto the allowlist, the staleness
  gate not firing, a malformed vintage failing **open**, a stale row keeping its tier, the ruling
  being softened, the IR range being dropped, TIDEMARK's denominator being replaced by Valquo's,
  the denominator warning falling off, a derived hurdle replaced by the audit's stale literal, the
  empty tiers being hidden, the band computed at raw n instead of effective n, a figure typed into
  the route, a caption written into the template, refusals no longer rendering, and the
  no-strategy-claim limit falling off.
* **The percentile-band control: bit-identical to TIDEMARK on 13 markets, max |diff| 0.000e+00.**
* **Zero trials.** `by_domain` is equity 234 / options 302 / infra 15 before and after.

---

## 7. Not done, named so it is not mistaken for done

* **NO LEDGER ROW WAS ADDED, and that is a decision rather than an omission.** `VALQUO_LEDGER.md`
  carries **no `MB` rows at all** — the audit landed on `main` without them, and the `MB` series
  belongs to the lane that produced it. Opening that series from this lane would collide with it.
  Recorded here instead; the ledger is the audit lane's to populate.
* **No `RESEARCH_LOG.md` row.** Zero trials, no hypothesis, no bar — a display surface, on the
  precedent that this lane's rows are not research rows.
* **The two empty tiers are shown with counts of 0, and that is TIDEMARK's finding rather than a
  rendering accident.** `ACTIONABLE` and `INDICATIVE` are empty; **15 of 16 markets are
  NOT INTERPRETABLE and 1 is REFUSED.** A test asserts the two stay empty and will fail if a
  market ever reaches an actionable tier — at which point this page's copy no longer describes it.
* **Nothing was re-measured, re-derived or re-run in TIDEMARK.** Its artifacts were read; its
  code was executed read-only to extract the table. No verdict of that project is restated here
  in different words, and none is adjudicated.
* **THE PAGE IS OWNER-ONLY UNDER PRIVATE MODE, AND THE ALLOWLIST WAS DELIBERATELY NOT WIDENED.**
  `saas/private.py`'s rule is deny-by-default — *"everything not named here is denied, so the
  failure mode of forgetting a route is 'the owner has to log in', never 'a stranger reads the
  book'"* — and `/methodology` is not in that allowlist either. **`/tidemark` therefore behaves
  exactly as `/methodology` does**, and a reader expecting "a public page" should know that is
  what MB25's *linked page* means here. Widening the allowlist is a security decision about a
  security-critical list, and it is not one a display item should take in passing.
* **The 2026-08-13 three-source disagreement on the forward track is untouched** and unrelated.
* **`.github/` untouched**, as instructed.
