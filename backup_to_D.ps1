...
$KEEP = @(
    ...
    @{ P = "data\free_analysis";            B = 1; Why = "the banked artifacts behind the published results. It was in the SKIP list as '0.07 GB of results JSONs recomputed by the scripts that wrote them'; MEASURED 2026-08-14 it is 0.80 GB and more than half of it is banked PANELS -- panel.pkl (the pre-B6 panel S19's C6 compares against), panel_corrected_69d.pkl, panel_s20_s21.pkl, panel_r5r6.pkl, S17_PRICES.pkl, m4_metrics_sink.pkl. A panel is a snapshot of a code state, so 'the script can rebuild it' is false the moment the script changes -- which is the whole of RUN_RULES rule 9. Several results are not recomputable at all without other data: O14_TICKFLOW_SIGNALS.json needs data\options_ticks, V6OPT_STAGE1/2.json need data\options_derived, and every seeded sweep is only interpretable against the research-log N it ran at." }
    ...
    @{ P = "data\options_ticks";            B = 2; Why = "ThetaData TICK prints -- 70,288,482 of them over 3,884 of the book's 3,885 alert-days, 4.40 GB measured 2026-08-14. Same vendor and the same argument as data\options, only stronger: D2 established the individual tier is 'personal use only, no business use' with lawful commercial access starting ~$250/mo plus OPRA firm registration, so this is not a re-download anyone can simply repeat. ThetaData also serialises a single account, so a re-mine cannot be parallelised out of its runtime. It is the sole input to O10, O18 and O14 -- the only intraday execution evidence the project owns. Named here because an ALLOWLIST is silent about anything unnamed, and silence is indistinguishable from a decision to drop it." }
