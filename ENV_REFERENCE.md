...
| `TRADIER_ACCOUNT_1_LABEL` … `_10_LABEL` | you run more than one account and want each alert card to say which one it is for | any short name you will recognise in Discord, e.g. `Main`, `Roth`. **An account exists only when its `LABEL` *and* `TOKEN` are both non-empty** — either alone is ignored. Gaps in the numbering are fine and do not renumber the rest. The label is posted verbatim, and a label carrying a phrase the dip register forbids (e.g. `Recovery Fund`) causes that account's dip card to be refused rather than published. |
| `TRADIER_ACCOUNT_1_TOKEN` … `_10_TOKEN` | with the matching `_LABEL` | a **data/sandbox** token. It is read only to decide whether the account exists and is never retained, returned or logged — the routing layer holds no credential, which is why it cannot reach an order endpoint. Never a token you would mind being unable to trade with. |
| `TRADIER_ACCOUNT_1_ID` … `_10_ID` | optional, with the pair above | the broker's account number, carried so a card can be traced back to an account. Not a credential. |
