# PREREG — SC-1: were this project's stated priors calibrated?

## EXECUTOR'S ACCEPTANCE — 2026-08-20

**ACCEPTED.** `PREREG_DRAFT_scout1_prior_calibration.md` (Frontier Scout lane, committed inside
the batch `5da7660` and therefore *not* a register) is adopted **verbatim** as this register.
**Everything from the horizontal rule below is the draft's text, byte-identical** — no bar, rule,
class, verdict state, void condition or expectation has been altered, added or removed. This
block is additive disclosure only, and it exists because the draft itself requires two things of
whoever adopts it: re-read the live counters, and commit ALONE.

**COMMITTED ALONE, markdown only, zero `.py`, as a strict git ancestor of every measurement
commit.** The draft's own presence on `main` inside a three-file batch is not that, which is
precisely why this file exists rather than the draft being promoted in place.

**COUNTERS RE-READ from `research_log.detail()` at acceptance, not quoted:** equity **234**,
options **305**, infra **18**. The draft's stated charge of "infra 17 → 18" is **stale as it
predicted it would be** — another lane booked an infra trial while the draft sat. **This register
charges infra 18 → 19.** Equity and options are untouched. Infra `N` gates no published claim.

### Two defects in the draft's own instrument, declared BEFORE any aggregate is read

Reported here rather than repaired, on this project's own precedent: `MB1` ran a kill condition
verbatim from its audit and reported that the rule did not follow; `O17C4` ran three registered
bars and reported that two of them did not measure what they were written to measure; `MB23`
ported a criterion's *correction* and kept the committed version visible beside it. **Running a
register as written is what surfaces its defects. Both are run as written and both are reported
either way.**

**D1 — §5's power arithmetic uses the wrong variance.** It bounds "per-pair Brier variance" at
0.25 and takes ≈0.09, then forms the detectable gap as `2·sqrt(0.09/n)·k`. But the primary
statistic is the **gap** `mean(p) − mean(y)`, whose standard error is driven by `Var(p − y)`, not
by the variance of the Brier score `(p − y)²`. For probabilities in the range this record states
and a roughly even outcome base rate, `Var(p − y)` runs nearer **0.15–0.25** than 0.09 — so §5's
figures are **optimistic**, i.e. the design is coarser than the draft advertises. **Consequence:
none to any verdict**, because §5 also instructs the executor to print the exact line from
`power_gate.state()` on the realized `n` before reading any aggregate, and that line is computed
from the **empirical** standard error of the realized `p − y` series. Both the draft's a-priori
figure and the empirical one are reported.

**D2 — C2 can fire for a reason that is not a defect.** It requires the extraction's per-item
right/wrong tallies to reproduce a write-up's own `Expectations N right, M wrong` line
**exactly**, with more than 2 mismatches sending the study back to stage 0. But those write-ups
score **every** expectation they stated, while §1 makes only expectations carrying **numeric**
odds scoreable — a write-up that scored 8 expectations of which 5 carried odds will mismatch by
construction. **C2 is run exactly as written and its result reported**; alongside it the same
comparison is reported restricted to the numerically-scoreable subset, labelled as the diagnostic
it is. If the as-written control fires, the finding is about C2's specification, not about the
extraction — and the register says so here, in advance, rather than after seeing which way it went.

### One thing the executor checked and did not change

The draft's §1 pools **register expectations** and **audit-item priors** into one universe without
separating them. They are different populations with different authors. The draft does not
distinguish them and neither does this run — but `source_file` is a banked column, so the split
is recoverable by anyone from the per-pair table, and it is **reported as a facet with no
verdict**.
