...
def test_landing_renders_the_sample_and_survives_having_none():
    ...
    try:
        ...
        html = c.get("/landing").data.decode()   # "/" redirects to /app since 2026-09-14
    ...
    try:
        c = create_saas_app(CONFIG).test_client()
        r = c.get("/landing")   # "/" redirects to /app since 2026-09-14
        assert r.status_code == 200
        html = r.data.decode()
...
def test_landing_never_500s_when_the_showcase_blows_up():
    ...
    try:
        r = create_saas_app(CONFIG).test_client().get("/landing")   # "/" -> /app since 2026-09-14
